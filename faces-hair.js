/*
 * faces-hair.js — hair silhouettes for the WHO IS THAT face generator.
 *
 * The hair path data below is copied (and rescaled) from faces.js by ZenGM
 * (https://github.com/zengm-games/facesjs), used under the Apache License 2.0.
 * Original SVGs are authored on a 400x600 canvas; here they are mapped onto this
 * project's 256x256 head via FACES_HAIR_TRANSFORM. No warranty; see Apache-2.0.
 *
 * Modifications: extracted <path d> only, stripped inline fill/stroke, and wrapped
 * in a transform so the hair frames this project's (differently proportioned) head.
 */
(function () {
  // Maps faces.js 400x600 coords onto this project's head: head centres + top-of-skull aligned,
  // head width matched. Uniform scale keeps the hair shapes undistorted.
  const FACES_HAIR_TRANSFORM = "translate(50.5 22) scale(0.387)";
  // faces.js strokes are ~4px in its space; /0.387 keeps the rendered weight near the face contour
  // and lets the stroke scale naturally with any head resize (no non-scaling-stroke needed).
  const STROKE = 7.6;

  // ---- Hair locks ----------------------------------------------------------
  // Hand-authored "hair layer" shapes (authored on a 512x512 canvas), each with four part layers:
  //   hair  - the silhouette (themed fill + ink outline)
  //   dark  - a depth/shadow blob
  //   shine - a highlight sweep
  //   lines - flowing internal strand lines (toggleable)
  // Two ways they're used:
  //   1. As registry detail on base pieces: dark/shine/lines (NOT the base hair) clipped to the
  //      style silhouette, adding sculpting + flow lines without a doubled edge.
  //   2. renderLock(): a freely-placed lock instance (the studio Hair Designer). Draws ALL parts,
  //      themed + transformed (x/y/scale/rotation) so the user can compose hair by dragging locks on.
  const LOCKS = {
    sideSwoop: {
      hair: ["M105 166c50-78 174-104 256-51 52 34 61 96 36 150-26 58-89 88-149 103 37-34 54-79 43-119-10-36-46-62-86-64-39-2-74 16-100 45 0-22 0-43 0-64z"],
      dark: ["M195 151c69-29 148-13 188 44 15 22 19 45 15 69 22-51 11-113-38-147-70-49-180-34-235 38 21-4 45-5 70-4z"],
      shine: ["M157 156c49-38 116-48 178-23-60-7-114 5-164 43-15 11-31-7-14-20z"],
      lines: ["M173 171c63-22 124-8 162 38", "M218 189c36 24 54 64 49 113"]
    },
    curtainBangs: {
      hair: ["M80 144c57-57 128-82 215-70 76 10 124 54 137 128 9 51-6 102-43 151-5-87-50-147-113-164-4 84-35 143-96 177 22-71 13-131-27-178-30 27-53 64-73 109-13-65-13-115 0-153z"],
      dark: ["M278 83c80 12 132 60 145 132 6 33 3 65-11 96-5-76-48-133-119-166-58-27-127-25-201 11 51-62 112-83 186-73z"],
      shine: ["M138 137c53-35 113-47 180-32-61 0-111 14-152 44-17 12-44 1-28-12z"],
      lines: ["M154 176c42 38 55 92 37 158", "M275 187c35 39 52 91 49 157", "M266 146c-5 85-27 147-67 186"]
    },
    messyTufts: {
      hair: ["M101 205c17-70 63-109 139-116l-7-65 48 64c29 1 56 8 82 21l46-42-15 68c40 32 55 79 45 141-54-46-108-69-162-67-59 2-117 31-176 86-8-34-8-64 0-90z"],
      dark: ["M118 224c53-56 113-85 180-86 58-1 103 20 135 63-8-39-31-70-68-93l15-41-44 38c-21-9-42-14-65-15l-38-66 10 69c-69 8-113 46-125 131z"],
      shine: ["M155 162c34-28 76-43 127-44-53 11-93 29-121 56-13 13-26 1-6-12z"],
      lines: ["M164 196c45-33 97-47 157-40", "M246 96c-17 35-19 71-6 109", "M357 122c-34 31-52 67-54 108"]
    },
    longSideLock: {
      hair: ["M174 73c67-28 153-7 197 49 52 65 40 158-23 227-40 43-68 83-59 121-56-21-81-71-59-125 15-36 45-64 66-98 25-40 25-85-5-112-42-37-111-21-166 49-17-50 1-90 49-111z"],
      dark: ["M294 118c47 30 56 87 26 138-24 40-65 70-79 116-10 33 6 72 48 98-63-22-90-76-64-133 19-43 66-77 77-119 11-41-11-75-52-88-37-12-80-1-128 37 45-63 120-82 172-49z"],
      shine: ["M194 92c54-17 110-4 151 32-50-25-100-30-151-13-20 7-24-12 0-19z"],
      lines: ["M263 132c40 30 45 76 14 132", "M248 321c-21 47-8 89 40 124"]
    },
    softWaveCap: {
      hair: ["M77 191c26-73 95-118 177-118 97 0 164 60 185 154 8 37 2 77-18 119-29-64-81-103-154-116-75-13-140 3-195 49-8-36-6-65 5-88z"],
      dark: ["M86 205c38-53 93-82 166-84 81-2 143 34 185 107-18-94-86-155-183-155-83 0-151 45-177 118-11 23-13 52-5 88 5-25 10-49 14-74z"],
      shine: ["M131 143c45-29 98-39 159-30-55 3-103 18-143 45-20 13-35-3-16-15z"],
      lines: ["M121 216c45-44 98-67 160-68", "M236 148c50 10 89 39 118 88", "M168 235c71-25 141-17 210 24"]
    },
    curlyForelock: {
      hair: ["M116 166c46-75 139-107 219-78 72 27 111 101 92 179-16 67-71 105-134 101-48-3-87-34-92-78-5-42 23-76 65-80 29-3 53 14 59 40-33-14-63-7-76 16-13 24 5 54 39 60 41 7 80-19 90-66 12-54-21-101-74-114-67-17-132 21-165 96-19-25-27-50-23-76z"],
      dark: ["M303 113c69 18 105 79 90 142-13 53-58 86-109 80-34-4-55-30-43-57-9 14-9 31 1 48 15 26 47 43 84 40 47-4 86-42 99-93 21-80-18-157-91-185-80-30-173 3-218 78-4 26 4 51 23 76 39-75 94-118 164-129z"],
      shine: ["M154 147c39-46 91-65 154-56-55 4-101 25-137 62-17 18-31 10-17-6z"],
      lines: ["M180 184c43-53 91-73 144-58", "M260 231c33-8 58 5 73 39"]
    },
    shortCrop: {
      hair: ["M65 220c24-85 91-134 182-146 83-11 153 20 202 92 22 33 33 69 33 110-57-42-124-61-202-56-81 5-149 35-204 90-15-34-18-64-11-90z"],
      dark: ["M83 226c64-52 137-79 221-78 71 1 130 26 177 74-10-52-45-100-94-126-82-44-216-20-284 44-25 23-38 52-38 80-7 26-4 56 11 90 2-27 5-55 7-84z"],
      shine: ["M122 165c50-37 110-55 180-53-62 10-115 30-158 60-19 13-43 7-22-7z"],
      lines: ["M119 232c30-35 60-61 89-79", "M202 219c21-43 48-77 81-102", "M294 214c13-38 38-70 75-95", "M378 223c14-27 35-49 64-67"]
    },
    spikyFringe: {
      hair: ["M73 212c48-83 115-126 201-130l20-61 27 68c39 9 75 27 108 56l51-14-34 58c26 38 31 82 16 132-42-60-87-96-135-107l-18 90-47-92c-25 6-50 20-76 41l-49 109-6-73c-24 22-43 47-58 75-14-62-14-113 0-153z"],
      dark: ["M95 221c51-65 111-98 179-100 62-2 118 24 166 78l39-68-51 14c-33-29-69-47-108-56l-27-68-20 61c-91 5-156 52-178 139z"],
      shine: ["M136 175c44-39 95-59 153-59-56 14-100 37-132 70-12 12-37 3-21-11z"],
      lines: ["M161 225c40-38 80-60 119-64", "M308 124c-8 47-6 88 7 123", "M400 173c-39 23-66 51-81 84"]
    },
    highPonytail: {
      hair: ["M119 166c42-58 104-88 186-89 74-1 128 30 162 93 25 46 26 99 5 159-35-58-77-92-126-102-43 62-107 96-193 102 49-38 76-78 81-121-39 9-78 31-118 67-13-44-12-80 3-109z", "M348 111c64-35 117-25 158 30-55 3-93 22-113 58-20-37-35-65-45-88z"],
      dark: ["M321 105c72 11 123 55 153 132-9-81-72-160-168-160-82 1-144 31-186 89-15 29-16 65-3 109 16-38 39-73 69-104 35-36 80-58 135-66z"],
      shine: ["M156 140c48-31 102-43 162-36-55 5-101 20-139 46-19 13-40 4-23-10z"],
      lines: ["M174 194c57-46 115-64 175-56", "M347 119c34 8 62 27 84 57"]
    },
    angularArc: {
      hair: ["M91 184c47-80 120-121 218-124 59-2 112 12 160 42-53 19-90 47-110 85 44 8 75 32 92 71-60-14-112-8-156 19-47 28-84 74-112 138-2-76 15-135 50-176-47 4-92 31-135 82-23-53-25-99-7-137z"],
      dark: ["M119 184c62-57 131-85 207-84 51 1 99 14 143 39 0-12 0-24 0-37-48-30-101-44-160-42-98 3-171 44-218 124-18 38-16 84 7 137 4-54 11-99 21-137z"],
      shine: ["M141 142c50-34 106-51 168-50-58 9-106 29-146 59-17 13-39 6-22-9z"],
      lines: ["M145 215c47-44 99-71 156-80", "M283 117c-38 46-55 92-52 139", "M354 189c-52 26-91 67-118 125"]
    },
    crescentSweep: {
      hair: ["M88 210c30-76 91-121 183-134 77-11 143 16 181 78-62-17-123-14-181 10-57 24-108 63-152 114-18-22-28-45-31-68z"],
      dark: ["M126 203c46-57 100-89 164-97 65-8 119 9 162 48-39-67-116-92-205-75-79 15-134 59-159 131 6 23 16 45 31 68 2-26 4-51 7-75z"],
      shine: ["M135 165c45-36 101-55 168-58-59 12-108 34-146 68-18 16-41 7-22-10z"],
      lines: ["M142 220c55-49 116-79 184-90", "M227 124c49 4 88 24 116 60", "M185 246c65-35 134-46 207-31"]
    },
    taperedCurtain: {
      hair: ["M181 74c66-2 125 19 176 64-19 42-39 89-59 140-18 47-29 99-33 156-24-75-54-135-90-180-35-44-48-84-39-121 5-24 20-44 45-59z"],
      dark: ["M200 83c60 1 112 22 157 55-51-21-99-22-145-2-33 15-53 41-59 78-7-36-14-63-17-81 7-25 28-43 64-50z"],
      shine: ["M190 101c38-12 78-8 119 12-40-7-75-2-105 14-24 13-37-17-14-26z"],
      lines: ["M214 95c-19 57-11 117 24 179", "M281 120c-34 62-47 133-40 213", "M170 151c43 48 68 105 74 171"]
    },
    splitFangBang: {
      hair: ["M120 120c48-53 98-65 151-36 43 24 73 72 90 146-36-30-70-41-101-32-14 43-27 86-40 130-22-64-54-112-96-145-10-27-11-48-4-63z", "M304 104c36 22 62 61 80 117-31-22-58-29-82-19-8 34-17 68-28 103-20-56-48-99-84-128 23-45 61-69 114-73z"],
      dark: ["M140 122c48-40 92-48 132-25 34 19 61 58 80 119-33-46-71-70-113-72-40-2-78 12-115 42-9-30-4-51 16-64z"],
      shine: ["M158 113c35-18 73-19 114-2-38-4-70 3-97 21-18 12-35-9-17-19z"],
      lines: ["M161 147c34 36 57 80 68 132", "M272 132c-34 47-51 101-52 162", "M318 144c24 28 42 63 54 106"]
    },
    cheekCurl: {
      hair: ["M336 88c69 29 104 95 84 170-18 67-77 112-143 103-51-7-85-43-79-88 5-39 38-66 80-63 27 2 49 18 61 43-39-15-68-9-86 17-16 24 2 54 36 60 44 8 88-22 100-70 14-56-16-112-86-145 8-14 19-23 33-27z"],
      dark: ["M344 113c48 33 66 87 45 140-20 51-68 83-116 73-33-7-51-31-42-58-6 15-4 31 6 47 18 30 55 49 96 42 45-8 80-45 91-95 17-78-19-145-88-174-10 6-21 15-33 27 14-3 27-4 41-2z"],
      shine: ["M326 107c36 11 62 32 78 64-24-25-51-41-81-48-20-5-22-21 3-16z"],
      lines: ["M352 133c35 37 45 82 29 134", "M264 225c35-9 62 3 80 36", "M256 306c43 23 88 13 118-28"]
    },
    napeFlip: {
      hair: ["M164 146c-51 45-73 105-59 180 10 55 48 97 111 126-33-54-30-107 9-158 36-47 33-100-9-136-17-15-34-19-52-12z"],
      dark: ["M180 161c-37 43-51 91-41 145 8 43 32 79 73 108-29-54-23-102 16-145 27-31 23-78-12-111-14-7-26-6-36 3z"],
      shine: ["M153 180c18-22 41-34 69-36-27 11-47 26-60 45-10 15-25 7-9-9z"],
      lines: ["M161 178c-31 52-37 109-18 172", "M206 183c27 43 25 88-6 136", "M150 333c6 44 28 77 66 100"]
    },
    scallopBangRow: {
      hair: ["M88 182c20-66 78-104 166-105 85-1 145 36 171 104-31-13-56-10-75 8-22 20-45 20-69 0-24-20-49-20-73 1-23 20-46 20-68 1-17-16-34-19-52-9z"],
      dark: ["M112 169c34-41 82-62 145-64 70-2 126 23 168 76-26-69-86-105-171-104-88 1-146 39-166 105 8-7 16-11 24-13z"],
      shine: ["M141 122c40-20 88-26 144-17-50 2-92 13-126 32-21 12-41-3-18-15z"],
      lines: ["M138 184c16-38 43-62 81-72", "M224 189c15-44 45-70 90-78", "M315 184c25-31 56-48 93-51"]
    },
    ribbonTail: {
      hair: ["M284 42c-71 38-107 96-97 173 6 49 38 84 61 127 28 54-10 101-119 133 66 9 125-7 174-47 51-42 63-91 35-146-25-49-42-86-28-126 10-31 36-59 75-86-25-17-58-27-101-28z"],
      dark: ["M252 61c-48 34-69 82-62 144 6 49 37 83 58 124 20 39 2 75-55 108 44-14 77-35 100-62 29-35 31-72 6-111-45-71-35-141 31-209-27-8-53-6-78 6z"],
      shine: ["M265 62c25-14 54-19 86-14-31 13-56 31-74 54-12 15-34-5-12-40z"],
      lines: ["M293 54c-55 48-75 102-61 163", "M238 246c45 49 38 101-22 156", "M290 115c-32 44-38 85-18 124"]
    },
    hookSideLock: {
      hair: ["M192 60c77-8 145 30 179 101 32 68 13 144-57 201-36 29-55 62-49 101-51-22-77-58-71-103 5-37 33-66 68-95 39-33 48-82 18-121-30-38-79-39-133-2-8-34 7-66 45-82z"],
      dark: ["M212 80c63 1 114 33 141 87 26 52 14 111-37 163-26 27-43 55-51 83-13-54 6-96 57-136 36-29 42-79 14-115-36-47-96-57-180-30 1-24 20-42 56-52z"],
      shine: ["M209 80c50-4 95 11 136 45-47-23-91-28-133-15-23 7-31-20-3-30z"],
      lines: ["M270 111c39 34 48 79 27 135", "M262 315c-19 45-10 84 28 119", "M180 117c58-24 110-18 156 17"]
    },
    cowlickSprout: {
      hair: ["M220 181c5-58 29-104 72-138 6 52-4 98-31 138 43-27 90-33 141-17-38 36-84 50-137 42 30 34 40 76 31 125-41-31-62-71-64-119-29 36-67 58-116 68 19-45 54-78 104-99z"],
      dark: ["M239 179c28-39 44-84 49-136-40 36-63 82-68 138-50 21-85 54-104 99 37-16 72-40 106-72 9 48 34 89 74 123-16-60-35-109-57-152z"],
      shine: ["M231 159c16-36 37-65 65-88-17 38-37 71-59 98-14 17-26 10-6-10z"],
      lines: ["M233 185c2-46 17-85 46-118", "M261 198c42-18 82-21 120-10", "M231 211c-25 31-57 51-96 61"]
    },
    roundedPuffSide: {
      hair: ["M116 199c-11-57 19-108 78-130 67-25 142-3 181 54 36 53 30 125-15 174-51 55-141 65-205 18-40-30-53-69-39-116z"],
      dark: ["M143 194c11-64 54-103 129-116-27-15-54-18-81-9-56 20-86 72-75 130-14 47-1 86 39 116-15-39-19-79-12-121z"],
      shine: ["M176 104c42-21 89-23 141-5-48-3-88 6-121 27-24 16-44-6-20-22z"],
      lines: ["M145 216c18-55 53-91 105-108", "M228 101c52 13 91 46 117 99", "M187 284c50 28 103 25 159-10"]
    },
    underWave: {
      hair: ["M91 279c43-39 94-56 153-51 66 6 121 39 166 99-56-24-110-25-162-3-51 22-101 57-150 105-18-54-20-104-7-150z"],
      dark: ["M115 283c48-26 100-35 157-26 54 9 100 32 138 70-45-60-100-93-166-99-59-5-110 12-153 51-13 46-11 96 7 150 2-52 8-101 17-146z"],
      shine: ["M136 268c42-20 90-25 144-14-50 1-91 11-125 31-23 13-43-5-19-17z"],
      lines: ["M126 312c43-33 91-52 145-58", "M205 329c57-24 117-21 179 8", "M105 360c45-44 99-75 164-92"]
    },
    // ---- Long framing locks (hair-shadow folded into dark, soft-line into lines) ----
    fullLongFrame: {
      hair: ["M81 206c-3-58 20-107 68-147 52-43 128-53 191-25 63 28 97 83 97 156 0 54-18 114-52 178-25 46-48 82-70 108 17-71 13-129-12-174-11 60-39 110-85 151 19-73 15-134-12-183-29 36-65 63-108 82 24-67 19-123-17-146z", "M431 204c17 72 11 139-18 201-17 36-42 61-75 74 35-60 45-118 30-174-17-61-10-107 21-138 18 6 32 18 42 37z"],
      dark: ["M104 209c-13-59 5-110 54-153 47-41 117-50 182-22-77-9-136 11-178 59-40 45-54 98-42 159 13 63 2 118-34 165 21-65 27-134 18-208z"],
      shine: ["M154 72c51-37 113-47 184-29-67 1-121 17-162 50-23 19-45-4-22-21z"],
      lines: ["M152 88c-43 52-58 111-43 176", "M211 60c-42 54-54 117-36 189", "M286 61c40 47 53 105 40 175", "M369 117c-36 58-44 124-24 198", "M125 302c-3 57-20 100-51 130", "M306 302c21 48 22 96 3 146"]
    },
    leftCascade: {
      hair: ["M250 42c-72-2-129 24-169 77-34 45-45 100-33 165 9 46 5 91-12 135 39-20 67-53 84-100 9 64-4 118-39 161 63-26 105-78 125-156 15-56 13-104-6-145-17-35 0-66 50-137z"],
      dark: ["M114 120c36-46 82-72 136-78-40 43-61 77-62 102-1 18 6 39 19 64-34-29-71-23-109 18-38 41-52 95-43 161-13-38-17-74-12-108-11-62 13-118 71-159z"],
      shine: ["M117 108c31-32 69-50 113-53-42 13-76 34-102 64-15 17-30 3-11-11z"],
      lines: ["M154 89c-39 52-51 113-35 181", "M203 68c-31 53-39 105-23 155", "M104 252c24 58 19 114-15 168", "M160 272c6 62-11 116-52 161"]
    },
    rightCascade: {
      hair: ["M262 42c72-2 129 24 169 77 34 45 45 100 33 165-9 46-5 91 12 135-39-20-67-53-84-100-9 64 4 118 39 161-63-26-105-78-125-156-15-56-13-104 6-145 17-35 0-66-50-137z"],
      dark: ["M398 120c-36-46-82-72-136-78 40 43 61 77 62 102 1 18-6 39-19 64 34-29 71-23 109 18 38 41 52 95 43 161 13-38 17-74 12-108 11-62-13-118-71-159z"],
      shine: ["M395 108c-31-32-69-50-113-53 42 13 76 34 102 64 15 17 30 3 11-11z"],
      lines: ["M358 89c39 52 51 113 35 181", "M309 68c31 53 39 105 23 155", "M408 252c-24 58-19 114 15 168", "M352 272c-6 62 11 116 52 161"]
    },
    centerPartWaves: {
      hair: ["M75 191c-6-56 13-103 57-139 48-39 116-49 185-29 67 19 106 65 118 139 7 45 3 97-13 157-15 58-42 107-79 147 27-71 24-130-8-177-10 62-39 114-86 157 22-74 15-138-22-191-28 48-68 83-119 105 24-69 14-126-33-169z"],
      dark: ["M252 31c69 0 121 24 157 73 30 41 39 92 27 153-9-83-45-140-109-172-63-31-128-26-195 16 32-47 72-70 120-70z"],
      shine: ["M150 66c49-31 104-39 167-25-59 1-108 14-147 41-22 15-42-3-20-16z"],
      lines: ["M252 41c-21 82-28 158-20 229", "M185 67c-47 57-62 122-45 196", "M319 68c45 59 55 128 31 207", "M130 270c13 59 2 111-34 157", "M336 286c21 52 17 105-12 158"]
    },
    longSStrand: {
      hair: ["M279 47c-71 16-117 55-138 117-17 51-6 90 33 117 34 24 40 55 18 94-18 32-48 57-91 75 66 10 121-6 164-48 45-45 57-99 36-162-17-51-5-91 36-120-7-33-26-58-58-73z"],
      dark: ["M241 65c-48 24-81 59-100 104-17 50-6 87 33 112 31 21 40 49 28 85 48-39 60-81 35-128-35-65-26-122 42-191-13 2-25 8-38 18z"],
      shine: ["M228 76c23-17 48-28 75-33-31 21-54 45-68 71-9 17-30 4-7-38z"],
      lines: ["M269 58c-62 47-83 99-63 156", "M208 244c44 43 36 92-25 147", "M244 110c-35 45-42 87-21 127"]
    },
    splitSideLocks: {
      hair: ["M117 86c40-43 92-63 156-58 62 5 107 32 136 80 24 40 29 91 14 153-11 46-12 94-2 145-43-42-67-98-72-168-24 15-45 39-62 72-6-63 5-113 33-150-45-27-92-31-143-12-42 16-69 50-80 101-13 62-6 130 22 204-55-45-83-105-85-181-3-78 25-140 83-186z"],
      dark: ["M153 92c42-35 94-50 158-43-81 11-139 47-175 107-36 59-44 132-24 217-48-68-53-139-15-212 14-26 32-49 56-69z"],
      shine: ["M154 76c45-25 95-31 150-16-51 0-93 10-127 30-23 14-44-1-23-14z"],
      lines: ["M195 63c-56 56-77 126-63 211", "M312 62c52 57 66 123 42 200", "M350 241c6 74 27 128 64 163", "M118 264c2 76 20 132 55 168"]
    },
    ribbonWaveLeft: {
      hair: ["M262 53c-70 2-124 29-163 81-36 48-44 104-24 169 15 49 10 98-17 146 71-19 116-65 134-137 12-46 9-87-10-123 28-1 56 9 84 31-15-67-16-123-4-167z"],
      dark: ["M126 138c34-43 79-70 136-85-26 37-35 77-27 122-64-17-111 5-141 65-17 35-20 75-9 122-36-94-22-168 41-224z"],
      shine: ["M131 121c29-28 66-47 112-58-36 24-66 50-89 78-16 19-42-1-23-20z"],
      lines: ["M203 82c-41 49-55 104-42 166", "M142 209c37 54 43 111 19 171", "M98 274c7 57-4 104-33 142"]
    },
    ribbonWaveRight: {
      hair: ["M250 53c70 2 124 29 163 81 36 48 44 104 24 169-15 49-10 98 17 146-71-19-116-65-134-137-12-46-9-87 10-123-28-1-56 9-84 31 15-67 16-123 4-167z"],
      dark: ["M386 138c-34-43-79-70-136-85 26 37 35 77 27 122 64-17 111 5 141 65 17 35 20 75 9 122 36-94 22-168-41-224z"],
      shine: ["M381 121c-29-28-66-47-112-58 36 24 66 50 89 78 16 19 42-1 23-20z"],
      lines: ["M309 82c41 49 55 104 42 166", "M370 209c-37 54-43 111-19 171", "M414 274c-7 57 4 104 33 142"]
    },
    longCapLocks: {
      hair: ["M82 184c2-63 28-111 77-145 55-38 124-43 207-14 50 18 80 56 91 113 9 49-2 105-32 169-24 52-37 103-39 154-40-42-55-91-46-148-28 53-68 97-119 133 34-71 39-132 15-183-32 50-76 84-132 103 28-68 21-129-22-182z"],
      dark: ["M118 181c-16-64 1-116 51-158 55-35 121-38 198-10-87-9-151 13-193 66-41 52-51 116-28 191 14 48 5 91-27 130 19-82 18-155-1-219z"],
      shine: ["M167 48c49-25 101-29 155-12-55-1-99 11-133 35-22 15-44-10-22-23z"],
      lines: ["M167 67c-39 55-50 117-31 187", "M229 40c-34 73-40 147-18 223", "M311 49c35 66 40 136 16 211", "M389 114c-29 57-37 118-23 184", "M118 282c10 53 2 101-25 144", "M250 300c8 57-3 103-34 139"]
    },
    extraLongPair: {
      hair: ["M158 48c-53 26-88 65-104 118-17 56-8 120 27 191 19 38 23 75 12 110 50-25 78-70 83-135 4-49-9-91-40-126 28-13 55-12 82 3-36-65-56-119-60-161z", "M354 48c53 26 88 65 104 118 17 56 8 120-27 191-19 38-23 75-12 110-50-25-78-70-83-135-4-49 9-91 40-126-28-13-55-12-82 3 36-65 56-119 60-161z"],
      dark: ["M106 153c19-51 52-86 101-105-33 44-48 88-44 132-31-11-56-3-75 23-28 38-31 91-10 159-43-85-34-161 28-209z", "M406 153c-19-51-52-86-101-105 33 44 48 88 44 132 31-11 56-3 75 23 28 38 31 91 10 159 43-85 34-161-28-209z"],
      shine: ["M114 132c16-31 39-54 70-70-27 31-44 62-50 93-4 23-31 2-20-23z", "M398 132c-16-31-39-54-70-70 27 31 44 62 50 93 4 23 31 2 20-23z"],
      lines: ["M149 72c-36 65-43 129-20 193", "M363 72c36 65 43 129 20 193", "M130 263c20 72 9 132-33 178", "M382 263c-20 72-9 132 33 178"]
    }
  };
  // Display names + ordering for the studio palette.
  const LOCK_CATALOG = [
    ["sideSwoop", "Side Swoop"], ["curtainBangs", "Curtain Bangs"], ["messyTufts", "Messy Tufts"],
    ["longSideLock", "Long Side Lock"], ["softWaveCap", "Soft Wave Cap"], ["curlyForelock", "Curly Forelock"],
    ["shortCrop", "Short Crop"], ["spikyFringe", "Spiky Fringe"], ["highPonytail", "High Ponytail"],
    ["angularArc", "Angular Arc"],
    ["crescentSweep", "Crescent Sweep"], ["taperedCurtain", "Tapered Curtain"], ["splitFangBang", "Split Fang Bang"],
    ["cheekCurl", "Cheek Curl"], ["napeFlip", "Nape Flip"], ["scallopBangRow", "Scallop Bang Row"],
    ["ribbonTail", "Ribbon Tail"], ["hookSideLock", "Hook Side Lock"], ["cowlickSprout", "Cowlick Sprout"],
    ["roundedPuffSide", "Rounded Puff Side"], ["underWave", "Under Wave"],
    ["fullLongFrame", "Long Wavy Frame"], ["leftCascade", "Left Cascade"], ["rightCascade", "Right Cascade"],
    ["centerPartWaves", "Center-Part Waves"], ["longSStrand", "Long S-Strand"], ["splitSideLocks", "Split Side Locks"],
    ["ribbonWaveLeft", "Ribbon Wave L"], ["ribbonWaveRight", "Ribbon Wave R"], ["longCapLocks", "Long Cap + Locks"],
    ["extraLongPair", "Extra-Long Pair"]
  ];
  // style concept -> lock concept (auto overlay default)
  const LOCK_FOR = {
    messy: "messyTufts",
    cropped: "shortCrop",
    sidePart: "sideSwoop",
    bob: "curtainBangs",
    curls: "curlyForelock",
    coily: "softWaveCap",
    locs: "longSideLock"
  };
  // Each lock authored on 512x512; map its hair-mass over this head's crown/fringe (clip keeps it on
  // the silhouette). Centre (~256,200 of the lock) -> crown (~128,95); slight per-style nudges.
  const DEFAULT_LOCK_TF = "translate(12.8 5) scale(0.45)";
  const LOCK_TF = {
    sidePart: "translate(20 4) scale(0.45)",   // bias the swoop slightly across the part
    locs: "translate(8 2) scale(0.46)",         // let the long lock reach a touch lower/wider
    bob: "translate(12.8 8) scale(0.45)"        // curtain bangs sit a hair lower over the brow
  };
  function lockShade(hex, f) {
    const n = parseInt(String(hex).replace("#", ""), 16);
    if (Number.isNaN(n)) return hex;
    const c = (sh) => Math.max(0, Math.min(255, Math.round(((n >> sh) & 255) * f)));
    return `rgb(${c(16)},${c(8)},${c(0)})`;
  }
  // A freely-placed lock instance:
  //   { lock, x, y, scale, rot, lines, mirror, outline, fill, dark, shine, line }
  // x/y are 0-100 (% of the 256 head box) -> the lock's centre; null colours derive from hair.
  // mirror flips it horizontally; outline = 'none' (no outline), a hex (custom), or unset (= ink).
  const LOCK_BASE_K = 0.42; // scale=1 -> a roughly head-sized lock
  function clamp(n, lo, hi) { return Math.max(lo, Math.min(hi, n)); }
  function hasPaint(value) {
    return !!value && value !== "none" && value !== "transparent" && value !== "rgba(0,0,0,0)";
  }
  function lockStrokeScale(inst, ctx) {
    const raw = Number(inst && inst.scale);
    const local = Number.isFinite(raw) ? raw : 1;
    const global = Number(ctx && ctx.outlineScale) || 1;
    return clamp(Math.pow(local, 0.38) * global, 0.55, 1.65);
  }
  function lockTransform(inst) {
    const kNum = LOCK_BASE_K * (Number(inst.scale) || 1);
    // optional non-uniform vertical scale (compositor layers carry scaleX/scaleY; legacy locks
    // stored a single uniform `scale`, which stays the default)
    const kyNum = LOCK_BASE_K * (Number.isFinite(Number(inst.scaleY)) ? Number(inst.scaleY) : (Number(inst.scale) || 1));
    const sx = ((inst.mirror ? -1 : 1) * kNum).toFixed(3);
    const sy = kyNum.toFixed(3);
    const hx = (((Number(inst.x)) || 50) / 100 * 256).toFixed(1);
    const hy = ((inst.y == null ? 32 : Number(inst.y)) / 100 * 256).toFixed(1);
    const rot = Number(inst.rot) || 0;
    return `translate(${hx} ${hy}) scale(${sx} ${sy}) rotate(${rot}) translate(-256 -256)`;
  }

  // The classic catalogue-lock renderer. Public renderLock/renderLockPart are compatibility
  // wrappers over renderHairPiecePart (the generic piece renderer), which routes lock:* layers
  // back here — one implementation, one entry point.
  function renderClassicLockPart(inst, ctx, mode) {
    const partMode = mode || "full";
    // Pen-tool / freehand lock: inst.d is a raw path string authored in portrait (256x256) space.
    // inst.strokes (optional) are interior strand lines. dx/dy nudge it; no lock-box transform.
    if (inst && inst.d) return renderDrawnLockPart(inst, ctx, partMode);
    const lock = inst && LOCKS[inst.lock];
    if (!lock) return "";
    inst = inst || {};
    const hair = (ctx && ctx.hair) || "#3a2418";
    const fill = inst.fill || (ctx && ctx.fill) || hair;
    const massFill = (ctx && ctx.massFill) || "#000";
    const ink = (ctx && ctx.ink) || "#1f2330";
    const outline = inst.outline === "none" ? "none" : (inst.outline || ink);
    const outlined = hasPaint(outline);
    const darkC = inst.dark || lockShade(hair, 0.5);
    const shineC = inst.shine || lockShade(hair, 1.3);
    const lineC = inst.line || (ctx && ctx.seam) || lockShade(hair, 0.62);
    const strokeScale = lockStrokeScale(inst, ctx);
    const seamBaseCutoff = clamp(Number(ctx && ctx.seamBaseCutoff) || 0, 0, 0.8);
    let body = "";

    if (partMode === "mass") {
      (lock.hair || []).forEach((d) => { body += `<path d='${d}' fill='${massFill}' stroke='none'/>`; });
    } else if (partMode === "fill") {
      // Solid fill only. Per-lock dark/shine/lines overlays trace each lock's silhouette, so where
      // locks OVERLAP (e.g. horn buns) the underlying lock's edges ghost through the one on top —
      // reading as line artefacts. The outer shape is carried by the rim; texture by the base hair
      // silhouette. (The studio's single-lock "full" preview still shows the shading.)
      (lock.hair || []).forEach((d) => { body += `<path d='${d}' fill='${fill}' stroke='none'/>`; });
    } else if (partMode === "outline") {
      if (!outlined) return "";
      (lock.hair || []).forEach((d) => { body += `<path d='${d}' fill='none' stroke='${outline}' stroke-width='${(5.8 * strokeScale).toFixed(2)}' stroke-linejoin='round' stroke-linecap='round'/>`; });
    } else if (partMode === "seam") {
      if (!hasPaint(lineC)) return "";
      const seamWidth = ((ctx && ctx.seamWidth) || 2.4) * strokeScale;
      const seamOpacity = (ctx && ctx.seamOpacity) == null ? 0.22 : ctx.seamOpacity;
      (lock.hair || []).forEach((d) => { body += `<path d='${d}' fill='none' stroke='${lineC}' stroke-width='${seamWidth}' stroke-linejoin='round' stroke-linecap='round' opacity='${seamOpacity}'/>`; });
    } else {
      (lock.hair || []).forEach((d) => { body += `<path d='${d}' fill='${fill}' stroke='${outlined ? outline : "none"}' stroke-width='${(5.8 * strokeScale).toFixed(2)}' stroke-linejoin='round'/>`; });
      (lock.dark || []).forEach((d) => { body += `<path d='${d}' fill='${darkC}' opacity='0.22'/>`; });
      (lock.shine || []).forEach((d) => { body += `<path d='${d}' fill='${shineC}' opacity='0.2'/>`; });
      if (inst.lines !== false) (lock.lines || []).forEach((d) => { body += `<path d='${d}' fill='none' stroke='${lineC}' stroke-width='${(4.4 * strokeScale).toFixed(2)}' stroke-linecap='round' stroke-linejoin='round' opacity='0.5'/>`; });
    }

    if (!body) return "";
    if (partMode === "seam" && seamBaseCutoff > 0) {
      const clipId = `lock-seam-cut-${String((ctx && ctx.seed) || "0").replace(/[^a-zA-Z0-9_-]/g, "_")}`;
      const height = 576 - seamBaseCutoff * 512;
      body = `<clipPath id='${clipId}'><rect x='-96' y='-96' width='704' height='${height.toFixed(1)}'/></clipPath><g clip-path='url(#${clipId})'>${body}</g>`;
    }
    return `<g transform='${lockTransform(inst)}'>${body}</g>`;
  }

  function renderLockPart(inst, ctx, mode) {
    return renderHairPiecePart(inst, ctx, mode || "full");
  }

  function renderLock(inst, ctx) {
    return renderHairPiecePart(inst, ctx, "full");
  }

  // Renders a hand-drawn (pen-tool) lock: its `d` is already in portrait coordinates, so we just
  // fill + outline it and lay optional interior strand lines (`strokes`) on top.
  function renderDrawnLockPart(inst, ctx, mode) {
    const partMode = mode || "full";
    if (partMode === "full") return renderDrawnLock(inst, ctx);
    const hair = (ctx && ctx.hair) || "#3a2418";
    const fill = inst.fill || (ctx && ctx.fill) || hair;
    const massFill = (ctx && ctx.massFill) || "#000";
    const lineC = inst.line || (ctx && ctx.seam) || lockShade(hair, 0.62);
    const seamWidth = ((ctx && ctx.seamWidth) || 1.5) * lockStrokeScale(inst, ctx);
    const seamOpacity = (ctx && ctx.seamOpacity) == null ? 0.22 : ctx.seamOpacity;
    const dx = Number(inst.dx) || 0;
    const dy = Number(inst.dy) || 0;
    let body = "";
    if (partMode === "mass") body = `<path d='${inst.d}' fill='${massFill}' stroke='none'/>`;
    if (partMode === "fill") body = `<path d='${inst.d}' fill='${fill}' stroke='none'/>`;
    if (partMode === "outline" && inst.outline !== "none") {
      const outline = hasPaint(inst.outline) ? inst.outline : ((ctx && ctx.ink) || "#1f2330");
      body = `<path d='${inst.d}' fill='none' stroke='${outline}' stroke-width='${(2.4 * lockStrokeScale(inst, ctx)).toFixed(2)}' stroke-linejoin='round' stroke-linecap='round'/>`;
    }
    if (partMode === "seam" && hasPaint(lineC)) {
      body = `<path d='${inst.d}' fill='none' stroke='${lineC}' stroke-width='${seamWidth.toFixed(2)}' stroke-linejoin='round' stroke-linecap='round' opacity='${seamOpacity}'/>`;
    }
    if (partMode === "detail") {
      // shading + interior strand strokes without the outline — used by the merged compositor so
      // drawn pieces keep their texture while the exterior contour comes from the union rim.
      const darkC = inst.dark || lockShade(hair, 0.5);
      if (inst.shade) body += `<path d='${inst.d}' fill='${darkC}' opacity='0.16'/>`;
      if (inst.lines !== false && Array.isArray(inst.strokes) && inst.strokes.length) {
        const clipId = `dlockd-${String((ctx && ctx.seed) || "0").replace(/[^a-zA-Z0-9_-]/g, "_")}`;
        const strands = inst.strokes
          .map((d) => `<path d='${d}' fill='none' stroke='${lineC}' stroke-width='${(1.7 * lockStrokeScale(inst, ctx)).toFixed(2)}' stroke-linecap='round' stroke-linejoin='round' opacity='0.5'/>`)
          .join("");
        body += `<clipPath id='${clipId}'><path d='${inst.d}'/></clipPath><g clip-path='url(#${clipId})'>${strands}</g>`;
      }
    }
    if (!body) return "";
    return dx || dy ? `<g transform='translate(${dx} ${dy})'>${body}</g>` : body;
  }

  function renderDrawnLock(inst, ctx) {
    const hair = (ctx && ctx.hair) || "#3a2418";
    const fill = inst.fill || (ctx && ctx.fill) || hair;
    const ink = (ctx && ctx.ink) || "#1f2330";
    const outline = inst.outline === "none" ? "none" : (inst.outline || ink);
    const outlined = hasPaint(outline);
    const dx = Number(inst.dx) || 0;
    const dy = Number(inst.dy) || 0;
    const lineC = inst.line || lockShade(hair, 0.62);
    const darkC = inst.dark || lockShade(hair, 0.5);
    const strokeScale = lockStrokeScale(inst, ctx);
    let body = `<path d='${inst.d}' fill='${fill}' stroke='${outlined ? outline : "none"}' stroke-width='${(2.4 * strokeScale).toFixed(2)}' stroke-linejoin='round' stroke-linecap='round'/>`;
    if (inst.shade) body += `<path d='${inst.d}' fill='${darkC}' opacity='0.16'/>`;
    if (inst.lines !== false && Array.isArray(inst.strokes) && inst.strokes.length) {
      // Clip the interior strand lines to the drawn shape so they never spill past its outline.
      const clipId = `dlock-${(ctx && ctx.seed) || "0"}`;
      const strands = inst.strokes
        .map((d) => `<path d='${d}' fill='none' stroke='${lineC}' stroke-width='${(1.7 * strokeScale).toFixed(2)}' stroke-linecap='round' stroke-linejoin='round' opacity='0.5'/>`)
        .join("");
      body += `<clipPath id='${clipId}'><path d='${inst.d}'/></clipPath><g clip-path='url(#${clipId})'>${strands}</g>`;
    }
    return dx || dy ? `<g transform='translate(${dx} ${dy})'>${body}</g>` : body;
  }



  const HAIR = {
    messy: [{m:"fs",d:"M150.74 72.13C150.71 72.56 153.9 74.79 157.93 77.14L165.18 81.37C162.45 83.01 160.01 84.07 157.99 84.79C155.97 85.51 152.55 86.47 150.37 86.92C148.2 87.38 141.86 89.08 136.27 90.7C130.69 92.32 123.08 94.52 119.35 95.59C115.63 96.65 110.43 98.1 107.79 98.8C105.16 99.5 100.08 100.78 96.51 101.64C92.94 102.49 88 103.83 85.51 104.61C83.03 105.38 80.29 106.31 79.42 106.66C78.49 107.04 77.9 107.66 78.01 108.16C78.11 108.63 79.68 109.48 81.57 110.08C83.43 110.68 86.04 111.57 87.37 112.06C88.7 112.55 89.77 113.27 89.76 113.66C89.75 114.04 88.11 115.95 86.1 117.89C84.1 119.83 80.5 122.94 78.1 124.8C75.7 126.66 72.46 129.21 70.89 130.47C69.32 131.74 66.61 134.15 64.89 135.83C63.16 137.52 60.57 140.42 59.14 142.28C57.71 144.14 54.37 149.92 51.71 155.11C49.05 160.31 46.88 164.94 46.88 165.4C46.88 165.87 46.62 166.31 46.3 166.39C45.99 166.47 45.54 167.47 45.32 168.62C44.94 170.54 45.03 170.76 46.46 171.3C47.71 171.78 48.01 172.24 48.01 173.72C48.01 174.73 47.64 179.36 47.19 184.02C46.74 188.67 45.96 195.02 45.47 198.12C44.97 201.22 44.19 206.04 43.74 208.83C43.29 211.63 42.67 217.21 42.37 221.24C42.07 225.27 41.37 231.66 40.82 235.44C39.91 241.78 39.9 242.39 40.74 243.34C41.3 243.97 41.9 246.34 42.29 249.44C42.64 252.23 42.93 257.06 42.93 260.16C42.94 263.26 43.19 268.72 43.5 272.28C43.8 275.85 44.42 280.42 44.87 282.44C45.32 284.45 45.83 287.82 46.01 289.91C46.31 293.62 46.37 293.74 48.29 294.56C49.38 295.03 50.26 295.66 50.26 295.97C50.26 296.28 51.34 297.3 52.66 298.24C54.23 299.37 55.74 299.95 57.03 299.95C58.35 299.95 59.41 299.52 60.22 298.67C60.89 297.96 62.89 294.46 64.67 290.9C66.45 287.33 68.34 283.78 68.87 283C69.4 282.22 70.98 279.43 72.38 276.8C73.78 274.16 75.6 269.84 76.42 267.21C77.24 264.57 78.15 261.02 78.44 259.31C78.73 257.61 79.38 251.13 79.89 244.93C80.39 238.73 81.31 231.11 81.94 228.01C82.56 224.91 83.74 220.35 84.57 217.88C86.06 213.45 86.11 213.39 88.62 212.6C90.01 212.16 94.83 211.64 99.33 211.44L107.51 211.09C108.95 203.25 109.54 201.59 110.33 200.96C111.15 200.31 112.16 200.15 114 200.37C115.39 200.53 118.95 201.27 121.89 202.01C124.84 202.74 129.79 204.54 132.89 206.01C135.99 207.47 139.67 208.8 141.07 208.96C143.55 209.25 143.62 209.22 144.19 207.35C144.66 205.82 145.1 205.41 146.44 205.27C147.37 205.17 149.64 205.65 151.5 206.32C153.36 207 158.19 209.23 162.22 211.28C166.25 213.32 172.09 216.67 175.19 218.72C178.29 220.77 181.72 223.21 182.81 224.14C183.89 225.07 186.43 227.01 188.45 228.45C190.46 229.89 192.56 231.08 193.1 231.09C193.75 231.1 194.09 230.68 194.09 229.84C194.1 229.14 192.83 225.46 191.27 221.66C189.72 217.86 188.61 214.63 188.82 214.47C189.03 214.32 189.79 214.19 190.51 214.19C191.24 214.19 193.73 214.7 196.06 215.32C198.39 215.94 201.62 216.97 203.25 217.61C204.88 218.25 206.85 218.69 207.62 218.6C208.97 218.43 209.04 218.23 209.21 214.19C209.3 211.86 209.69 209.63 210.05 209.23C210.42 208.83 211.87 208.51 213.26 208.52C214.66 208.53 218.46 209.02 221.72 209.6C224.98 210.19 229.55 211.43 231.87 212.36C234.2 213.29 239.02 215.38 242.59 217C246.16 218.63 250.47 220.47 252.18 221.1C253.88 221.72 255.53 222.45 255.84 222.73C256.15 223.01 257.3 223.23 258.38 223.24C260.31 223.24 260.35 223.19 259.99 221.39C259.79 220.38 258.37 217.27 256.84 214.47C255.3 211.68 252.99 207.56 251.7 205.31C250.41 203.06 249.36 201.09 249.36 200.94C249.36 200.78 250.56 200.5 252.04 200.3C253.51 200.11 255.03 199.66 255.41 199.32C255.8 198.97 256.24 196.71 256.4 194.31L256.69 189.94C259.97 189.37 263.84 189.37 267.41 189.57C270.97 189.77 274.62 190.13 275.51 190.36C276.74 190.68 278.09 192.17 281.01 196.43C283.15 199.53 285.34 203.02 285.88 204.19C286.42 205.36 287.18 206.31 287.57 206.31C287.99 206.3 288.27 205.55 288.27 204.46C288.26 203.45 288.6 201.49 289.01 200.09C289.43 198.7 290.89 195.35 292.27 192.66C294.39 188.48 294.91 187.84 295.75 188.28C296.29 188.57 298.89 190.97 301.53 193.61C304.17 196.24 307.43 200.11 308.78 202.21C310.13 204.3 312.41 208.61 313.85 211.79C315.29 214.97 316.97 219.42 317.57 221.66C318.18 223.91 319.05 227.66 319.5 229.98C319.96 232.31 320.75 239.29 321.25 245.49C321.76 251.7 322.41 258.17 322.7 259.88C322.99 261.58 323.9 265.14 324.72 267.77C325.54 270.41 327.36 274.72 328.76 277.36C330.16 280 331.74 282.79 332.27 283.56C332.8 284.34 334.69 287.89 336.47 291.46C338.25 295.03 340.25 298.52 340.92 299.23C341.73 300.09 342.79 300.51 344.11 300.51C345.41 300.52 346.91 299.93 348.48 298.81C349.8 297.87 350.88 296.85 350.88 296.54C350.88 296.23 351.77 295.59 352.85 295.13C354.78 294.3 354.84 294.18 355.14 290.47C355.31 288.38 355.82 285.02 356.27 283C356.72 280.98 357.34 276.42 357.65 272.85C357.95 269.28 358.2 263.82 358.21 260.72C358.21 257.62 358.5 252.8 358.85 250.01C359.24 246.9 359.84 244.53 360.4 243.9C361.24 242.95 361.24 242.34 360.32 236.01C359.77 232.23 359.08 225.84 358.78 221.81C358.47 217.77 357.86 212.19 357.4 209.4C356.95 206.61 356.17 201.78 355.68 198.68C355.18 195.58 354.41 189.23 353.96 184.58C353.51 179.93 353.14 175.3 353.14 174.29C353.14 172.81 353.43 172.34 354.69 171.86C356.11 171.32 356.2 171.11 355.83 169.19C355.6 168.04 355.16 167.03 354.84 166.96C354.52 166.88 354.26 166.43 354.26 165.97C354.26 165.5 352.01 160.74 349.26 155.39C346.51 150.04 342.51 143.16 340.38 140.09C338.24 137.03 334.54 131.7 332.15 128.25C329.76 124.8 326.68 120.07 325.32 117.75C323.95 115.42 322.51 112.63 322.11 111.54C321.48 109.82 321.55 109.09 322.6 105.76C323.26 103.67 323.82 101.26 323.83 100.4C323.85 99.55 323.53 98.57 323.13 98.22C322.73 97.88 320.37 96.82 317.88 95.87C315.4 94.92 311.47 93.27 309.14 92.2C306.82 91.14 304.09 89.6 303.08 88.78L301.25 87.29C303.09 81.51 303.5 78.67 303.5 75.87C303.5 72.07 303.37 71.5 302.52 71.52C301.97 71.53 300.51 72.36 299.27 73.38C298.03 74.4 294.99 76.65 292.5 78.39C290.02 80.14 285.58 82.71 282.63 84.12C279.69 85.53 275.75 87.22 273.89 87.88C272.03 88.55 269.24 89.43 267.69 89.86C265.11 90.56 264.72 90.53 263.18 89.56C262.25 88.97 260.47 87.44 259.23 86.15C257.99 84.87 256.27 83.45 255.42 83.01C254.57 82.57 253.17 82.22 252.32 82.22C251.28 82.23 250.02 82.96 248.51 84.43L246.26 86.62C237.08 82.84 231.3 80.83 227.5 79.72C222.71 78.32 219.47 77.7 216.93 77.7C214.38 77.7 213.26 77.92 213.25 78.41C213.25 78.79 214.28 80.13 215.55 81.37C216.82 82.61 218.06 84 218.31 84.47C218.66 85.13 217.95 85.03 215.02 84C212.97 83.28 206.34 81.45 200.29 79.94C194.24 78.43 185.23 76.55 180.27 75.77C175.3 74.98 169.72 74.05 167.86 73.69C166 73.33 162.7 72.82 160.53 72.55C158.36 72.28 155.28 71.9 153.69 71.71C151.47 71.44 150.79 71.53 150.74 72.13Z"}],
    cropped: [{m:"fs",d:"M200 95C320 95 340 185 340 210C320 210 320 215 315 210C310 205 300 195 265 195C250 195 225 200 200 200C175 200 150 195 135 195C100 195 90 205 85 210C80 215 80 210 60 210C60 185 85 95 200 95Z"}],
    sidePart: [{m:"fs",d:"M61.7 299.46L50.91 298.23C50.91 298.23 40.36 290.76 40.92 276.9C41.12 270.28 41.47 263.65 41.95 257.08C42.43 250.33 43.05 243.73 43.82 237.32C44.59 230.82 45.53 224.57 46.61 218.58C47.73 212.54 49 206.78 50.42 201.3C53.37 190.2 56.92 180.16 60.97 171.06C65.11 161.67 69.69 153.11 74.61 145.33C79.54 137.17 84.78 129.81 90.27 123.27C95.74 116.45 101.48 110.59 107.46 105.77C113.45 100.86 119.75 97.15 126.31 94.71C132.93 92.34 139.88 91.34 147.08 91.72L150.66 102.57C150.66 102.57 149.91 89.02 156.54 83.57C166.29 81.85 176.02 80.98 185.63 80.89C195.33 80.98 204.87 81.8 214.13 83.26C223.42 84.8 232.4 86.88 240.94 89.4C249.47 91.91 257.53 94.79 265.02 97.95C272.47 101.02 279.34 104.34 285.52 107.85C291.67 111.25 297.14 114.85 301.84 118.63C311.16 125.87 317.46 134.01 320 143.2C329.94 151.78 337.34 156.21 342.88 160.07C348.39 163.81 352.06 167.14 354.54 173.31C357.01 179.45 358.3 188.56 358.98 204C359.31 211.78 359.47 221.19 359.53 232.61C359.55 238.33 359.54 244.55 359.51 251.31C359.47 258.05 359.41 265.31 359.34 273.12C359.12 290.75 349.12 300.39 349.12 300.39L339.29 301.3C339.29 301.3 339.35 289.25 339.37 285.13C339.4 280.96 329.46 270.1 324.43 266.79C321.91 265.13 320.68 253.51 318.85 241.12C317.03 228.41 314.61 214.94 309.55 211.15C307.02 209.22 301.26 209 293.39 209.83C285.48 210.59 275.44 212.35 264.37 214.46C253.28 216.44 241.2 218.71 229.28 220.64C217.37 222.39 205.66 223.8 195.3 224.33C179.83 224.83 170.45 219.95 164.62 213.58C158.85 207.02 156.59 198.98 155.26 193.51C155.07 198.97 142.38 204.06 128.6 208.67C115.04 212.75 100.55 216.13 95.71 220.31C90.91 224.32 86.07 237.18 82.09 248.82C78.38 259.78 75.48 269.74 73.26 271.22C68.82 274.14 60.06 283.35 60.33 286.76C60.62 290.07 61.7 299.46 61.7 299.46Z"}],
    bob: [{m:"fs",d:"M60.7 303.6L50.52 306.33C50.52 306.33 40.18 299.58 40.05 280.08C38.93 269.01 38.32 258.62 38.08 249C37.88 239.41 38.05 230.66 38.5 222.69C39.01 214.77 39.81 207.61 40.86 201.13C41.97 194.65 43.31 188.82 44.82 183.57C48 172.99 51.73 164.66 55.36 158.29C59.07 151.81 62.56 147.35 65.25 144.71C70.63 139.34 78.38 133.58 87.42 126.28C96.36 118.88 106.49 110.1 116.71 99.11C127.39 88.13 139.22 82.49 149.56 79.56C159.94 76.92 168.81 76.65 173.41 76.44C182.61 76.17 199.02 93.13 199.02 93.13C199.02 93.13 219.64 76.8 228.95 77.4C233.62 77.77 242.72 78.38 252.82 81.26C262.99 84.39 274.36 90.08 283.84 100.66C293.43 111.33 303.57 120.04 312.76 127.43C322.19 134.78 330.39 140.63 336.01 146.01C338.82 148.66 342.43 153.21 346.15 159.76C349.81 166.2 353.49 174.58 356.48 185.1C357.89 190.33 359.12 196.12 360.09 202.53C361.01 208.94 361.68 216.02 362.04 223.82C362.38 231.67 362.43 240.28 362.16 249.7C361.86 259.16 361.22 269.34 360.1 280.17C359.88 299.59 349.41 306.39 349.41 306.39L339.18 303.74C339.18 303.74 340.9 292.51 341.35 288.56C341.73 284.52 333.32 272.35 328.87 268.51C326.6 266.61 323.21 255.15 318.59 242.44C313.36 228.83 306.9 213.68 301.3 208.65C295.64 203.42 281.51 201.24 268.76 200.25C256 199.13 244.66 199.39 244.66 199.39C244.66 199.39 237.37 190.29 235.86 188.75C235.1 187.98 235.76 189.14 239.52 194.3C243.3 199.38 250.21 208.18 261.88 222.1C251.55 222.53 235.5 217.37 222.1 211.79C208.86 205.93 198.35 199.86 198.35 199.86C198.35 199.86 188.4 205.92 175.75 211.72C162.98 217.19 147.55 222.2 137.3 221.7C147.64 208.01 153.74 199.35 157.07 194.34C160.39 189.25 160.97 188.09 160.28 188.86C158.89 190.38 152.39 199.38 152.39 199.38C152.39 199.38 141.25 199.1 128.87 200.17C116.53 201.08 102.95 203.14 97.72 208.25C92.53 213.18 86.91 228.09 82.27 241.53C77.99 254.16 74.67 265.63 72.35 267.56C67.75 271.49 58.91 283.94 59.19 288.04C59.51 292.07 60.7 303.6 60.7 303.6Z"}],
    curls: [{m:"fs",d:"M182.42 68.88C182.12 69.9 181.7 72.18 181.48 73.94C181.08 77.05 180.97 77.11 175.35 76.84C172.2 76.69 168.75 77.04 167.68 77.62C166.62 78.2 165.79 79.37 165.84 80.24C165.89 81.11 167.31 83.1 168.99 84.67C170.67 86.24 172.9 89.15 173.96 91.13C175.01 93.1 175.9 95.4 175.95 96.24C176 97.22 174.85 97.87 172.75 98.06C170.95 98.23 168.94 98.92 168.27 99.57C167.6 100.24 166.81 100.44 166.52 100.02C166.22 99.61 166.37 97.79 166.83 95.97C167.28 94.15 167.58 91.62 167.49 90.35C167.41 89.07 166.27 87.36 164.97 86.53C162.91 85.21 162.37 85.21 160.69 86.53C159.63 87.35 158.31 88.02 157.75 88.02C157.2 88.01 156.23 86.98 155.6 85.72C154.84 84.16 153.13 83.14 150.39 82.57C146.68 81.81 146.1 82.01 143.72 84.84C141.43 87.55 140.58 87.87 137.03 87.4C134.2 87.02 132.98 87.25 133 88.15C133.01 88.86 134 90.71 135.2 92.26C136.39 93.8 137.41 95.42 137.45 95.83C137.48 96.25 136.61 97.5 135.5 98.61C134.38 99.71 133.74 101.18 134.07 101.88C134.41 102.57 133.67 103.81 132.44 104.64C131.21 105.46 129.63 106.11 128.92 106.11C128.22 106.09 127.85 105.42 128.1 104.59C128.36 103.76 127.94 101.71 127.17 100.01C126.07 97.52 125.35 97.02 123.52 97.45C121.83 97.85 121.11 97.47 120.68 95.96C120.34 94.77 119.26 93.92 118.07 93.9C116.96 93.89 115.16 94.79 114.08 95.89C112.67 97.32 111.31 97.71 109.3 97.28C107.25 96.82 105.96 97.21 104.51 98.72C103.42 99.87 102.85 101.37 103.24 102.06C103.64 102.78 102.48 104.92 100.54 107.03C98.66 109.05 97.12 111.16 97.13 111.7C97.14 112.25 96.46 112.68 95.61 112.67C94.76 112.66 93.85 112.2 93.55 111.65C93.26 111.1 92.22 110.65 91.24 110.66C90.26 110.66 88.9 111.42 88.23 112.34C87.55 113.26 87 115.24 87 116.73C87 118.69 86.28 119.71 84.44 120.39L81.89 121.33C86.69 125.98 88.09 127.77 88.09 128.31C88.09 128.84 89.02 129.3 90.15 129.32C91.28 129.34 92.45 130.13 92.75 131.07C93.04 132.01 93.05 132.89 92.76 133.02C92.47 133.15 91.42 132.86 90.43 132.37C89.34 131.84 87.6 131.86 86.06 132.43C84.65 132.95 81.75 133.28 79.64 133.18C77.53 133.07 75.79 133.42 75.8 133.96C75.8 134.5 74.99 134.68 74 134.34C72.7 133.92 71.44 134.89 69.35 137.92C67.78 140.21 66.73 142.63 67.05 143.3C67.35 143.98 69.24 145.33 71.24 146.3C73.78 147.54 74.78 148.61 74.56 149.9C74.39 150.91 73.47 152.2 72.51 152.78C71.18 153.58 70.3 153.42 68.67 152.09C66.67 150.42 66.44 150.44 63.45 152.48C61.33 153.93 60.51 155.16 60.89 156.33C61.21 157.27 60.76 158.57 59.89 159.22C59.03 159.88 58.33 160.96 58.36 161.63C58.38 162.3 59.32 163.4 60.45 164.08C62.25 165.18 62.31 165.47 60.95 166.51C59.77 167.42 59.45 169.39 59.62 174.75C59.81 180.87 59.53 182.15 57.46 184.63C55.67 186.82 54.22 187.52 51.45 187.6C48.89 187.67 47.27 188.38 45.99 189.99C44.72 191.59 44.36 193.15 44.78 195.19C45.11 196.79 46.14 198.67 47.08 199.37C48.02 200.06 48.64 201.37 48.47 202.28C48.29 203.18 48.4 204.89 48.71 206.09C49.01 207.28 49.92 208.26 50.74 208.25C51.61 208.25 52.21 209.14 52.21 210.42C52.21 211.62 51.46 213.68 50.56 215.01C49.27 216.91 49.06 218.61 49.56 222.97C49.95 226.44 49.82 228.5 49.21 228.51C48.68 228.51 48.02 229.61 47.76 230.93C47.44 232.55 46.63 233.35 45.36 233.37C43.82 233.39 43.52 233.91 43.82 236.04C44.03 237.49 43.66 240.86 43 243.52C42.35 246.17 41.88 251.88 41.96 256.2C42.02 260.51 42.43 264.39 42.86 264.83C43.28 265.25 44.02 264.85 44.5 263.92C44.96 262.99 45.78 262.21 46.3 262.21C46.81 262.2 47.25 263.16 47.25 264.35C47.26 265.54 46.67 268.24 45.94 270.37C44.91 273.33 44.8 275.89 45.42 281.4C45.87 285.35 46.76 290.45 47.4 292.74C48.05 295.04 49.09 297.29 49.72 297.75C50.5 298.32 51.19 297.88 51.87 296.42C52.55 294.93 53.5 294.35 54.89 294.57C56.54 294.84 57.28 294.06 58.83 290.44C59.89 287.99 61.09 285.98 61.47 285.97C61.86 285.96 62.39 287.05 62.62 288.36C62.86 289.69 63.69 290.76 64.48 290.76C65.26 290.75 65.91 290.41 65.93 290.01C65.96 289.62 66.63 288.41 67.42 287.34C68.86 285.4 68.88 285.41 70.24 287.55C71 288.74 72.15 289.72 72.81 289.73C73.47 289.75 74.55 288.53 75.23 287.04C75.9 285.55 76.89 281.47 77.42 278.01C77.94 274.56 78.38 267.93 78.36 263.3C78.35 257.67 78.89 253.4 80.02 250.32C80.97 247.74 81.5 244.4 81.22 242.65C80.95 240.95 81.26 237.72 81.93 235.48C82.6 233.24 83.57 230.65 84.09 229.73C84.62 228.8 85.07 227.62 85.08 227.09C85.1 226.57 87.05 223.65 89.42 220.61C93.29 215.69 93.69 214.69 93.19 211.31C92.79 208.67 93.03 207.32 93.98 206.83C94.73 206.46 96.51 206.36 97.93 206.64C99.55 206.94 101.12 206.57 102.12 205.63C103 204.81 104.9 203.9 106.34 203.62C107.8 203.33 110.9 201.4 113.25 199.31C117.2 195.82 118 195.51 123.34 195.45C126.55 195.41 129.88 194.99 130.76 194.51C131.88 193.91 132.74 194.12 133.58 195.2C134.62 196.52 136.85 196.75 147.89 196.65C159.24 196.54 161.42 196.77 164.12 198.35C166.37 199.68 169.3 200.23 174.71 200.33C178.81 200.42 184.08 200.55 186.41 200.62C188.74 200.7 198.48 200.36 208.01 199.86C217.49 199.39 226.94 199.04 229.06 199.08C231.53 199.13 234.12 198.4 236.26 197.05C239.2 195.18 240.96 194.92 252.21 194.69C261.53 194.51 265.01 194.11 266.1 193.1C267.26 192.02 269.22 191.87 275.56 192.37C283 192.98 283.66 193.21 287.1 196.53C289.08 198.47 291.44 200.07 292.34 200.09C293.24 200.09 295.23 200.97 296.76 202.02C298.29 203.07 300.36 203.73 301.37 203.48C302.38 203.23 304.02 203.25 305.02 203.53C306.56 203.95 306.75 204.55 306.27 207.56C305.74 210.76 306.09 211.65 309.93 216.86C312.22 220.05 314.68 224.17 315.4 226.03C316.12 227.9 317.1 230.5 317.57 231.83C318.04 233.16 318.66 237.28 318.94 240.99C319.19 244.7 320.07 249.07 320.91 250.7C322.01 252.83 322.29 255.38 321.95 259.94C321.64 263.66 321.98 269.48 322.77 274.12C323.5 278.46 324.16 283.34 324.25 284.97C324.33 286.6 325.15 288.72 326.07 289.69C327.68 291.37 327.82 291.35 329.43 289.01C330.34 287.68 331.39 286.6 331.77 286.61C332.14 286.62 333.05 287.96 333.77 289.6C334.49 291.21 335.53 292.33 336.07 292.07C336.6 291.82 337.14 290.51 337.26 289.17C337.37 287.84 337.78 286.74 338.15 286.75C338.52 286.76 339.69 288.79 340.75 291.27C342.31 294.84 343.11 295.64 344.61 295.22C346.01 294.84 346.76 295.38 347.57 297.37C348.64 299.9 348.73 299.94 349.76 298.11C350.34 297.06 351.23 290.6 351.7 283.76C352.37 273.42 352.28 270.78 351.23 268.14C350.53 266.38 350.01 263.65 350.06 262.07C350.11 260.48 350.51 259.2 350.95 259.23C351.4 259.25 351.83 259.79 351.93 260.43C352.03 261.06 352.56 261.31 353.12 260.97C353.86 260.53 354.11 257.89 354.04 251.47C353.98 246.57 353.56 241.56 353.1 240.34C352.64 239.12 352.3 236.39 352.34 234.27C352.4 231.43 352.07 230.27 351.09 229.85C350.35 229.54 349.34 228.18 348.83 226.83C348.32 225.48 347.64 224.37 347.32 224.34C347 224.32 346.92 222.26 347.14 219.76C347.45 216.17 347.11 214.22 345.54 210.54L343.52 205.89C346.55 203.33 348.06 200.19 349.09 196.69C350.37 192.27 350.64 190.18 350.08 188.73C349.66 187.65 348.61 186.32 347.75 185.76C346.87 185.21 345.26 184.74 344.14 184.71C343.03 184.69 341.93 184.23 341.71 183.69C341.49 183.16 342.04 182.53 342.94 182.27C343.84 182.02 344.58 181.06 344.59 180.13C344.6 179.21 345.07 177.08 345.63 175.41C346.49 172.86 346.46 172.09 345.45 170.69C344.78 169.76 342.92 168.29 341.31 167.41C339.05 166.18 338.05 164.81 336.99 161.48C336.23 159.08 335.12 155.72 334.53 154C333.94 152.27 333.06 150.86 332.57 150.86C332.09 150.87 331.04 150.11 330.22 149.19C328.81 147.58 328.62 147.58 326.23 149.21C324.09 150.66 323.71 150.7 323.67 149.46C323.65 148.68 322.81 147.82 321.81 147.55C320.81 147.29 320.17 146.55 320.39 145.88C320.6 145.21 322.18 143.9 323.9 142.96C325.61 142.02 327.2 140.58 327.42 139.78C327.64 138.99 326.76 136.37 325.43 133.99C323.75 130.98 322.21 129.48 320.44 129.07C319.03 128.76 316.42 128.8 314.64 129.15C311.53 129.78 311.35 129.66 309.8 125.84L308.17 121.87C312.73 117.79 314.04 116.27 314.02 115.87C313.99 115.47 313.33 115.15 312.55 115.16C311.78 115.17 310.65 114.09 310.05 112.74C309.05 110.46 308.66 110.35 304.46 111.03C301.98 111.43 298.5 112.82 296.72 114.12C294.95 115.41 292.53 116.47 291.33 116.47C290.13 116.46 289.14 116.25 289.12 115.98C289.1 115.7 289.62 113.82 290.27 111.79C290.94 109.7 292.69 107.31 294.27 106.31C295.81 105.33 297.05 103.98 297.02 103.29C297 102.61 296.04 101.28 294.89 100.34C293.11 98.87 292.96 98.31 293.89 96.54C294.82 94.76 294.68 94.25 292.93 93.02C291.8 92.22 289.89 91.59 288.67 91.61C287.45 91.63 285.84 92.68 285.08 93.94L283.71 96.21C279.83 91.71 278.06 90.74 276.29 90.75C274.8 90.76 272.72 91.22 271.67 91.79C270.63 92.36 269.2 94.19 268.52 95.86C267.84 97.53 267.35 99.79 267.43 100.89C267.53 102.11 266.92 102.91 265.87 102.95C264.83 102.98 262.67 100.9 260.46 97.74L256.77 92.42C261.05 87.65 262.25 85.45 262.18 84.46C262.11 83.38 260.94 82.32 259.27 81.81C257.71 81.33 254.94 81.37 253.05 81.88C250.43 82.61 249.26 82.46 248.02 81.25C247.14 80.4 246.06 79.67 245.65 79.66C245.24 79.64 244.09 80.69 243.09 81.98L241.28 84.31C236.69 79.94 234.34 78.66 233.1 78.65C231.86 78.64 230.57 79.38 230.23 80.29C229.89 81.19 227.84 82.26 225.67 82.65C222.09 83.3 221.73 83.68 221.74 86.72C221.75 88.6 221.11 90.28 220.29 90.57C219.49 90.86 218.36 90.87 217.79 90.58C217.21 90.3 216.89 88.57 217.06 86.72C217.34 83.86 216.97 83.13 214.62 81.8C212.37 80.53 211.51 80.48 210.01 81.55C208.45 82.65 208.05 82.57 207.39 81.03C206.96 80.03 207.02 77.31 207.53 74.99C208.35 71.19 208.23 70.61 206.51 69.39C205.44 68.65 204.23 68.04 203.82 68.05C203.41 68.06 203.12 68.79 203.19 69.68C203.25 70.56 202.62 71.28 201.8 71.28C200.98 71.28 200.05 70.8 199.73 70.21C199.4 69.63 198.03 69.14 196.65 69.14C195.09 69.14 194.19 69.75 194.26 70.75C194.33 71.63 193.71 72.58 192.91 72.88C192.11 73.17 190.28 72.7 188.84 71.81C187.4 70.93 186.18 69.5 186.11 68.61C186.05 67.72 185.32 67 184.49 67C183.65 67 182.71 67.85 182.42 68.88Z"}],
    coily: [{m:"fs",d:"M60 310L50 310C50 310 10 310 10 260C10 120 80 70 200 70C320 70 390 120 390 260C390 310 350 310 350 310L340 310C340 310 340 295 340 290C340 285 330 270 325 265C320 260 315 220 310 210C303.68 197.35 250 200 200 200C150 200 96.32 197.35 90 210C85 220 80 260 75 265C70 270 60 285 60 290C60 295 60 310 60 310ZM200 70"}],
    longWaves: [{m:"fs",d:"M84.6,349.3c-2.9-2.7-5.7-5.4-8.7-8c-19.6-16.4-25.8-36.4-20.6-59.4c1.4-6.3,2.3-12.7,4.2-18.8 c1-2.4,3.1-4.5,5.9-5.6c5.1-1.7,6.9,0.1,7.5,4.8c3.4,26.2,3,52.9,10.4,78.8c0.7,2.6,1.5,5.2,2.3,7.8L84.6,349.3z M78.4,337 c-9.2-20-9.1-41.4-13.2-62.2h-1.3l-3.3,24.5c-2.5-5.8,0.8-26.7,5.5-30.9c2.5,16,5,31.5,7.4,46.9c0.8-0.5,1-1,1-1.3 c-1.1-17.2-2.2-34.5-3.4-51.8c-0.1-1-0.7-2.6-1.4-2.8c-1.3-0.3-3.4-0.2-4.2,0.5c-1.7,1.7-3.6,3.6-4.1,5.7 c-1.3,5.2-1.2,10.8-2.8,15.8c-2.7,8.6-2.2,17-0.8,25.5C59.9,319,67.3,329.8,78.4,337L78.4,337z"},{m:"fs",d:"M78.4,337c-11.2-7.2-18.6-17.9-20.5-30c-1.5-8.5-1.9-17,0.8-25.5c1.6-5.2,1.5-10.6,2.8-15.8 c0.5-2.1,2.4-4,4.1-5.7c0.8-0.7,2.9-0.9,4.2-0.5c0.7,0.2,1.3,1.8,1.4,2.8c1.2,17.3,2.3,34.5,3.4,51.8c0,0.6-0.4,1.1-1,1.3 c-2.4-15.5-4.9-30.9-7.4-46.9c-4.7,4.2-8,25-5.5,30.9l3.2-24.5h1.3C69.3,295.5,69.2,316.9,78.4,337z"},{m:"fs",d:"M378.1,263.2c-8.6-27.1-18.2-53.9-27.4-80.9c-6.7-20-19.4-38.5-36.9-53.7c-15.5-13.6-32.6-25.6-52.1-35.3 C249.2,87.2,236.4,82,221,82.5c-12.4,0.4-24.9,0.3-37.3,0c-11.3-0.4-22.6,1.7-32.6,5.9c-16.5,7-31.6,15.8-45,26 c-10.2,7.7-20.5,15.5-28.9,24.5c-12.5,13.6-20.9,29-27.1,45.3c-15.8,41.1-30.9,82.4-35.8,125.4c-1.2,10.3-2.8,20.8,2.4,30.9 c2.2,4.5,4.8,8.9,7,13.4c1.6,3,3.8,6.2,4.1,9.3c0.5,4.3,2.6,7,7,9.5c11.5,6.5,22.3,13.7,33.9,20c5.4,3,8.8,6.5,11.7,11 c6.4,9.9,14.5,19.3,24.2,27.4c1.3,0.9,2.8,1.7,4.2,2.4c-2.7-6.1-5.7-11.5-7.5-17.1c-2-5.8-2.9-11.9-4.3-17.8 c-0.3-1-0.7-2.7-1.2-4.8c-10.7-44.8-7.3-81-5.7-89.3c2.7-14,5.4-27.9,8.9-41.8c2.2-8.6,5.6-16.9,8.5-25.3c4.5,0.7,8,1.1,11.6,1.8 c2.8,0.6,5.4,1.4,7.9,2.4c4,1.7,7.7,4.5,12,5.1c18.7,2.8,37.4,4.9,56.9,7.3c1.8-7.8,3.4-14.8,5.3-22.2c1.7,7.9,3.2,14.9,4.7,21.7 c0.8,0.2,1.5,0.3,2.3,0.3c13.5-1.2,26.9-2.2,40.3-4c7.8-1.1,16.5-2.4,22.9-5.8c7.2-3.9,14.8-4.3,22.6-6.3c0.6,1.7,1.2,3,1.7,4.3 c8,21.1,13.5,43,16.4,65.1c4.9,37-2.3,67.3-6.4,87.2c-0.4,1.8-0.7,3.2-0.9,4.2c-3.5,12.3-3.1,24.6-12.3,35.1c1.7-0.2,3.4-1,4.5-2 c8-8.9,16.3-17.6,23.6-26.8c3.7-4.7,7.2-8.9,13.2-12c12.9-7,25.1-14.8,37.5-22.3c0.8-0.7,1.2-1.5,1.3-2.4c0.6-6.9,5.1-12.5,8-18.7 c3.2-6.9,7.5-14.2,7.6-21.4C388.6,306,384.8,284.4,378.1,263.2z M26.4,351.6l-1.1,0.3c-0.6-1.1-1.3-2.2-2-3.3 c-9.4-15.3-9.1-31.4-6.2-47.8c6.2-35.8,17.6-70.6,30.7-105c3.6-9.3,7.4-18.5,11.2-27.8C42.8,228.7,24.2,289.1,26.4,351.6z M100.9,422.8l-1.9,0.4c-3.7-4.6-7.8-9.2-11.4-14c-6.8-9.3-14.1-18.5-20.1-28.2c-10.7-17.2-12.3-35.9-11.1-54.8 c1.3-20.4,9.5-39.5,16.1-58.8c3.7-10.7,7.5-21.3,11.3-32c0.6-1.6,1.3-3.1,2-4.9l20.5,6.6C78.9,298.4,80.5,360.5,100.9,422.8z M304.4,421c-0.8,0.9-1.7,1.5-2.9,1.9c19.3-62.3,21.4-124.3-5.7-185.9l20.8-6.6c4.1,11.6,8.4,22.9,12.1,34.4 c5.2,16.6,10.6,33.3,14.9,50.1c3.5,14.4,3.4,29.2-0.4,43.4c-2.1,8.2-4.8,16.8-9.7,24.1C324.8,395.7,314.2,408.2,304.4,421z M371,366.3c-0.4,1-1.1,1.8-2.1,2.4c-4.1,2.7-8.3,5.5-12.7,8.1c-8.2,4.9-16.4,9.6-25.5,14c1.8-2.8,3.8-5.5,5.4-8.4 c2.3-4.1,5.2-8,6.4-12.2c6-21.4,7.2-43,0.9-64.2c-7.3-24-15.8-47.8-24-71.6c-1.1-3.3-1.3-5,3.1-6.5c5.4-1.9,5.6-3.6,4.1-9.9 c-3.8-15.4-12-29.2-21.8-42.6c-11.4-15.5-24.5-29.8-44.1-39.2c-15.1-7.2-30.8-14-48.5-16.4c-13.2-1.8-26.8-0.6-39,3.6 c-0.5,0.3-0.9,0.6-1.3,1c6.9-1.2,13.9-2.9,20.9-3.6c13-1.3,25.7-0.5,37.6,4.8c11.3,4.8,23.3,8.6,33.8,14.2 c20.4,10.8,33.1,27.1,44.8,43.8c7.8,11.2,12.1,23.5,16.1,35.9c0.4,1.7,0.7,3.5,0.8,5.2c-10.6,3.1-21.1,6.2-31.6,9.3 c-3.4,0.9-6.6,2.3-10,3.2c-3.9,1-6-0.3-7-3.6c-6.1-19.3-14.7-38.2-25.7-56c-1.9-3-4-6-6.3-8.9c5.6,10.5,11.8,20.8,16.6,31.6 c5.2,11.9,9.1,24.2,13.6,36.3c1,2.7-1.6,3.6-3.7,4.6c-8.4,4.1-17.8,6.1-27.6,6.9c-11.5,0.9-22.9,1.8-34.3,2.6c-1.1,0-2.1,0-3.1-0.2 c-1.8-13.5-3.8-26.9-5.6-40.4h-0.7l-5.4,40.6c-8.9-0.6-17.2-1-25.6-1.7c-13.1-1.1-26.4-1.7-38.2-7.4c-4-1.9-5.3-3.5-4.3-7.5 c4.9-19.5,14.2-37.8,24-56c1.7-3.1,3.9-6.2,5.3-9.6c-1.1,1.1-2,2.3-2.8,3.4c-12.2,18.8-21.8,38.7-28.6,59.2 c-2.3,6.8-4.6,7.7-12.7,4.8c-3.8-1.3-7.5-2.5-11.3-3.7c-7.6-2.7-15.3-5.1-22.8-7.9c-1-0.5-1.8-2.5-1.5-3.6 c3.7-15.6,10.7-30.5,20.9-44.1c11.7-15.4,24.7-29.9,44.3-39.6c8.4-4.2,17.8-7.3,26.7-11.1c1.3-0.6,2.5-1.3,3.7-2.1 c-25.2,7-46.8,18.3-62.8,36c-12.4,13.5-22.3,28.1-29.5,43.7c-2.5,5.5-3.8,11.3-5.5,17c-1,3.3,0.4,5.7,4.4,7.1 c1.6,0.4,3.2,1.2,4.9,1.9c-2.7,7.4-5.5,14.5-8,21.7c-5.2,14.6-10.7,29.2-15.4,44.1c-8.3,26-8.8,52.3-0.3,78.4 c1.7,5.4,6.4,10,9.7,14.9c0.4,0.6,0.7,1.2,1.6,2.9c-2.7-1.4-4.4-2.3-5.9-3.2c-9.9-5.9-19.9-11.8-29.9-17.7 c-3.7-2.2-5.4-4.8-5.7-8.6c-3.5-38.1,1.2-75.8,10.5-113.2c5.2-21.1,11.4-42.3,16.5-63.2c2.7-11.4,6.4-22.5,12.5-33.1 c7.1-12.2,18.4-21.6,30-30.6c18-14,37.2-26.6,60.8-34.1c5.2-1.8,10.6-2.7,16.2-2.9c16.2-0.3,32.5,0,48.7,0 c12.3,0,22.8,4.6,32.9,9.4c26,12.2,48.6,28.4,66.6,47.8c9.4,10.4,14.7,22.4,18,35.1c7.5,29.6,15.5,59,22.4,88.5 c6.9,29.1,7.7,58.6,8.1,88.1C373,358,371.8,362.1,371,366.3z M377.4,350.6c-0.1,0.1-0.6,0.2-1.9,0.5 c1.2-62.8-16.1-123.3-32.9-184.2c2,5.3,4.2,10.6,6.1,15.9c6.3,16.8,12.9,33.5,18.5,50.5c9.5,28.5,17.6,57.3,19.6,86.8 C387.6,331,382.7,340.9,377.4,350.6z"},{m:"fs",d:"M172.4,123.8c-25.4,7-46.9,18.3-62.8,35.9c-12.3,13.4-22.2,28.1-29.5,43.7c-2.4,5.4-3.8,11.4-5.5,17 c-1,3.3,0.4,5.7,4.4,7.1c1.6,0.6,3.1,1.2,4.9,1.9c-2.8,7.3-5.4,14.5-8,21.7c-5.2,14.7-10.8,29.4-15.4,44.1 c-8.2,26.1-8.8,52.4-0.3,78.4c1.7,5.3,6.4,9.9,9.7,14.9c0.3,0.6,0.6,1.1,1.6,2.9c-2.8-1.6-4.4-2.4-5.9-3.2 c-10-5.9-19.9-11.9-29.9-17.7c-3.8-2.2-5.4-4.7-5.7-8.6c-3.5-38.2,1.2-75.9,10.5-113.3c5.3-21.2,11.4-42.3,16.5-63.6 c2.8-11.5,6.4-22.6,12.5-33.1c7-12.1,18.4-21.4,30-30.6c17.9-13.9,37.2-26.5,60.8-34.1c5.2-1.7,10.6-2.6,16.2-2.9 c16.2-0.3,32.4-0.1,48.7,0c12.3,0,22.8,4.6,32.9,9.4c26,12.2,48.5,28.4,66.5,47.8c9.6,10.3,14.9,22.5,18,35.1 c7.5,29.5,15.5,58.8,22.4,88.5c6.7,29.1,7.6,58.6,8.1,88.1c0.1,4.2-1.1,8.3-2,12.5c-0.4,1-1.1,1.8-2.1,2.4 c-4.2,2.8-8.4,5.5-12.7,8.1c-8.2,4.9-16.4,9.6-25.5,14c1.8-2.8,3.8-5.5,5.4-8.4c2.3-4,5.2-7.9,6.4-12.2c6-21.2,7.2-42.8,0.9-64.2 c-7.1-24-15.7-47.7-24-71.6c-1.1-3.1-1.3-5,3.1-6.5c5.4-1.8,5.6-3.6,4.1-9.9c-3.8-15.4-12-29.3-21.8-42.6 c-11.3-15.5-24.5-29.8-44.1-39.2c-15.2-7.3-30.9-14-48.5-16.4c-13.2-1.8-26.7-0.5-39,3.7c-0.5,0.2-0.9,0.6-1.3,1L172.4,123.8z"},{m:"fs",d:"M172.1,123.9c6.9-1.2,13.9-2.9,20.9-3.6c13.1-1.3,25.7-0.5,37.6,4.8c11.1,4.9,23.3,8.7,33.8,14.2 c20.3,10.8,33,27.1,44.8,43.8c7.9,11.4,12.1,23.5,16.1,35.9c0.4,1.7,0.6,3.4,0.8,5.2c-10.5,3.1-21.1,6.1-31.6,9.3 c-3.4,1-6.5,2.4-10,3.2c-4,1.1-6-0.2-7-3.6c-6.1-19.3-14.7-38-25.7-56c-1.9-3-4-6-6.3-8.9c5.6,10.5,11.8,20.9,16.6,31.6 c5.2,11.9,9.1,24.1,13.6,36.3c1,2.8-1.5,3.6-3.7,4.6c-8.5,4.2-17.8,6.1-27.6,6.9c-11.4,0.9-22.8,1.7-34.3,2.6c-1,0-2-0.1-3.1-0.2 c-1.9-13.5-3.8-27-5.6-40.4h-0.8l-5.5,40.6c-8.8-0.6-17.2-0.9-25.6-1.7c-13.2-1.1-26.4-1.7-38.2-7.4c-4-1.8-5.3-3.5-4.3-7.5 c4.9-19.6,14.2-37.9,24-56c1.7-3.1,3.9-6.2,5.3-9.6c-1,1-1.9,2.2-2.8,3.4c-12.1,18.8-21.7,38.7-28.6,59.2 c-2.2,6.7-4.7,7.7-12.7,4.8c-3.7-1.3-7.5-2.4-11.3-3.7c-7.6-2.5-15.3-5-22.8-7.9c-1-0.4-1.8-2.4-1.5-3.6 c3.8-15.6,10.9-30.5,21.1-44.1c11.6-15.5,24.7-29.9,44.3-39.6c8.6-4.2,17.8-7.4,26.7-11.1c1.3-0.6,2.5-1.4,3.8-2.1L172.1,123.9z"},{m:"fs",d:"M295.8,237l20.7-6.8c4.1,11.7,8.5,23.2,12.1,34.5c5.3,16.6,10.6,33.3,14.9,50.1c3.5,14.3,3.4,29.1-0.4,43.4 c-2,8.2-4.9,16.8-9.7,24.1c-8.8,13.2-19.3,25.8-29.1,38.6c-0.7,0.8-1.6,1.6-2.9,1.9C320.8,360.7,323,298.7,295.8,237z"},{m:"fs",d:"M99,423.2c-3.8-4.6-7.9-9.1-11.4-13.9c-6.9-9.3-14.1-18.5-20.1-28.2c-10.7-17.3-12.3-35.8-11.1-54.9 c1.3-20.4,9.5-39.5,16.1-58.9c3.7-10.7,7.5-21.3,11.3-32c0.6-1.6,1.2-3.1,2-4.9l20.5,6.7c-27.4,61.3-25.7,123.3-5.4,185.7L99,423.2 z"},{m:"fs",d:"M59,168c-16.1,60.8-34.7,121.1-32.5,183.8l-1.1,0.3c-0.7-1.1-1.4-2.2-2-3.3c-9.4-15.3-9.1-31.5-6.2-47.8 c6.2-35.8,17.6-70.5,30.7-105C51.3,186.5,55.2,177.3,59,168z"},{m:"fs",d:"M375.5,351.1c1.2-62.9-16.1-123.5-32.7-184.2c2,5.3,4.2,10.6,6.1,15.9c6.2,16.9,12.8,33.6,18.5,50.5 c9.5,28.5,17.4,57.3,19.6,86.8c0.8,10.9-4.1,20.7-9.4,30.5C377.3,350.9,376.9,350.9,375.5,351.1z"}],
    locs: [{m:"fs",d:"M217.4 2.49C218.56 2.89 222.09 3.73 225.26 4.36C230.23 5.34 231.02 5.81 231.01 7.75C231 9.03 230.24 10.22 229.25 10.51C228.29 10.79 227.83 11.35 228.22 11.76C228.62 12.17 228.51 13.29 227.97 14.25C227.09 15.84 226.77 15.86 224.5 14.5C222.72 13.43 222.12 13.36 222.42 14.25C222.65 14.94 223.1 16.74 223.42 18.25C223.9 20.52 223.57 21.17 221.5 22C219.32 22.87 219.14 23.29 220.1 25.25C220.98 27.06 220.78 28.28 219.08 31.5C217.83 33.87 215.75 36.01 213.99 36.75C211.38 37.84 211 38.51 211 42C211 44.89 211.42 46 212.5 46C213.32 46 214.22 45.55 214.5 45C214.78 44.45 216.35 44 218 44C220.67 44 221.06 43.61 221.5 40.5C221.86 37.95 222.61 36.84 224.25 36.39C225.49 36.06 226.84 35.83 227.25 35.89C227.66 35.95 228 35.33 228 34.5C228 33.32 228.81 33.13 231.75 33.63C235.14 34.19 235.42 34.07 234.72 32.38C234.3 31.34 234.19 29.71 234.47 28.75C234.76 27.79 235.45 27.23 236 27.5C236.55 27.77 237 27.55 237 27C237 26.45 238.34 26.11 239.98 26.25C241.63 26.39 243.19 25.94 243.48 25.25C243.76 24.56 244.34 24.05 244.75 24.12C245.16 24.18 246.06 24.18 246.75 24.12C247.44 24.05 248.22 23.1 248.5 22C248.95 20.2 249.33 20.12 252.25 21.13C255 22.09 255.62 22.01 256.25 20.63C256.66 19.73 257.9 19 259 19C260.1 19 261 19.34 261.01 19.75C261.01 20.16 261.87 20.5 262.91 20.5C263.96 20.5 265.2 20.19 265.66 19.8C266.12 19.42 267.18 19.37 268 19.7C268.82 20.03 269.95 19.67 270.5 18.9C271.06 18.13 272.07 17.39 272.75 17.25C273.44 17.11 274.69 17.56 275.54 18.25C276.39 18.94 276.84 20.29 276.54 21.25C276.24 22.21 276.45 23.23 277 23.5C277.55 23.77 278.68 23.55 279.5 23C280.32 22.45 281.18 22.34 281.41 22.75C281.63 23.16 281.63 25.98 281.4 29C281.17 32.02 280.42 34.83 279.74 35.23C279.03 35.65 277.94 35.22 277.19 34.23C275.95 32.6 275.81 32.6 274.69 34.21C274.03 35.15 272.94 35.94 272.25 35.96C271.56 35.98 270.1 35.1 269 34C267.9 32.9 267 31.55 267 31C267 30.45 265.76 29.81 264.25 29.57C262.74 29.33 259.81 28.88 257.75 28.57C254.41 28.06 253.97 28.24 253.75 30.25C253.56 31.92 252.38 32.89 249.17 34C245.42 35.29 244.91 35.81 245.42 37.75C245.87 39.5 245.45 40.22 243.5 41C242.13 41.55 241 42.45 241 43C241 43.55 241.79 45.65 242.75 47.66C244.07 50.42 244.19 51.41 243.25 51.66C242.44 51.88 242.88 52.88 244.5 54.5C245.88 55.88 247.56 57 248.25 56.99C248.94 56.99 250.29 57.89 251.25 58.99C252.21 60.1 253.68 61 254.5 61C255.32 61 256.23 61.79 256.51 62.75C256.79 63.71 257.35 64.17 257.76 63.76C258.17 63.36 259.03 63.81 259.67 64.76C260.56 66.09 260.56 66.81 259.67 67.79C258.77 68.79 258.79 69.08 259.75 69.04C260.44 69.02 262.13 68.55 263.5 68C265.43 67.23 266.23 67.34 267 68.5C267.69 69.54 267.61 70.51 266.75 71.65C266.06 72.55 265.64 74.47 265.82 75.9C266.06 77.81 265.52 78.81 263.82 79.67C262.54 80.32 260.6 80.62 259.5 80.34C258.38 80.06 256.84 80.54 256 81.43C254.79 82.72 254.25 82.78 253.21 81.76C252.49 81.07 251.93 79.71 251.96 78.75C251.98 77.63 251.46 77.18 250.5 77.5C249.68 77.78 248.32 77.78 247.5 77.5C246.68 77.22 246.01 76.21 246.03 75.25C246.04 74.29 245.82 73.28 245.53 73C245.24 72.72 243.32 73.02 241.25 73.66C239.19 74.3 237.5 75.47 237.5 76.26C237.5 77.05 237 78.33 236.39 79.1C235.78 79.87 235.22 82.19 235.14 84.25C235.06 86.31 234.78 88.45 234.5 89C234.22 89.55 233.21 89.77 232.25 89.49C230.95 89.1 230.31 89.74 229.75 91.99C229.13 94.46 228.54 94.96 226.44 94.75C225.03 94.61 223.68 94.84 223.44 95.25C223.2 95.66 222.1 96 221 96C219.55 96 219.15 96.48 219.55 97.75C219.86 98.71 220.31 102.54 220.55 106.25C220.99 112.9 220.96 113 218.5 113C216.67 113 216 113.53 216 115C216 116.76 215.55 116.92 212.25 116.36C210.19 116 208.39 115.33 208.25 114.86C208.11 114.39 207.55 114 207 114C206.45 114 206.01 114.56 206.03 115.25C206.05 115.94 205.03 117.78 203.78 119.34C201.97 121.59 200.98 122.04 199 121.52C197.33 121.09 196 121.4 195 122.45C194.03 123.47 192.85 123.76 191.66 123.26C190.46 122.77 190.03 121.89 190.41 120.75C190.8 119.6 190.49 119 189.5 119C188.68 119 187.71 119.56 187.35 120.25C186.87 121.18 186.42 121.07 185.6 119.82C184.99 118.9 183.6 118.13 182.5 118.12C181.4 118.1 178.81 118.07 176.75 118.05C173.48 118.01 172.92 117.64 172.41 115.25C172.09 113.74 171.3 110.92 170.67 109C170.04 107.08 169.51 105.05 169.49 104.5C169.47 103.95 168.79 101.99 167.98 100.14C166.66 97.15 166.13 96.82 163.16 97.14C160.74 97.4 159.57 97.02 158.91 95.75C158.41 94.79 156.99 93.49 155.75 92.86C154.51 92.24 153.84 92.13 154.25 92.61C154.67 93.1 154.22 94.29 153.25 95.25C151.59 96.91 151.34 96.89 148.25 95C146.46 93.9 144.55 93.22 144 93.5C143.45 93.78 143 95.13 143 96.5C143 98.1 142.37 99.11 141.25 99.3C140.29 99.47 138.71 99.47 137.75 99.3C136.43 99.07 136 98.03 136 95C136 91.38 135.78 91.04 133.75 91.46C132.45 91.72 130.74 91.19 129.72 90.21C128.74 89.27 127.95 87.94 127.97 87.25C127.99 86.56 127.44 85.82 126.75 85.6C125.97 85.35 125.69 84.14 126 82.42C126.28 80.85 127.33 79.39 128.4 79.07C129.44 78.76 130.68 77.49 131.15 76.25C131.69 74.82 133.09 73.82 135 73.5C136.65 73.22 138 72.55 138 72C138 71.45 138.68 71 139.5 71C140.32 71 141.79 72.11 142.75 73.47C144.49 75.93 144.51 75.93 149.5 72.12L146.77 69.31C145.27 67.76 144.04 65.38 144.03 64C144.03 62.63 143.46 60.97 142.77 60.31C142.07 59.66 140.94 59.32 140.25 59.56C139.56 59.8 138.3 59.44 137.46 58.75C136.61 58.06 135.93 56.04 136 51L131.75 51.62C127.87 52.18 127.39 52 126.25 49.62C125.11 47.23 125.22 46.89 127.53 45.75C128.92 45.06 130.06 44.05 130.06 43.5C130.06 42.95 129.82 42.16 129.53 41.75C129.24 41.34 127.87 41.56 126.49 42.25C124.3 43.34 123.8 43.25 122.53 41.5C121.71 40.37 121.36 38.63 121.72 37.5C122.07 36.4 122.44 34.38 122.55 33C122.66 31.63 122.8 29.94 122.87 29.25C122.94 28.56 123.8 28 124.77 28C125.8 28 127.06 26.74 127.77 25C128.77 22.56 129.42 22.12 131.25 22.62C132.49 22.97 134.4 24.15 135.5 25.26C137.25 27.02 137.33 27.54 136.16 29.39C135.03 31.15 135.03 31.62 136.16 32.25C136.9 32.67 139.3 33.55 141.5 34.22C143.7 34.88 145.95 35.9 146.5 36.49C147.05 37.07 147.65 38.21 147.83 39.02C148.01 39.84 148.91 41.32 149.83 42.32C151.44 44.07 151.54 44.07 152.72 42.32C153.54 41.09 153.62 39.94 152.97 38.75C152.43 37.79 152 35.88 152 34.5C152 33.13 152.72 31.66 153.59 31.25C154.56 30.79 154.95 29.82 154.59 28.75C154.2 27.6 154.6 26.82 155.75 26.48C156.71 26.19 157.95 26.39 158.5 26.92C159.13 27.53 160.33 27.54 161.75 26.94C163.51 26.21 164.27 26.4 165.25 27.83C166.19 29.21 167.24 29.51 172.5 28.43L168.23 26.46C165.04 24.99 163.97 23.93 163.98 22.25C163.99 21.01 164.45 19.77 165 19.5C165.55 19.23 165.76 18.44 165.46 17.75C165.16 17.06 165.85 15.6 166.99 14.5C168.13 13.4 169.05 11.82 169.04 11C169.03 10.18 170.25 8.93 171.76 8.24C173.27 7.55 175.06 6.98 175.75 6.99C176.44 7 177.11 8.01 177.25 9.25C177.39 10.49 178.51 11.95 179.75 12.5C180.99 13.05 182.01 14.29 182 15.25C182 16.21 181.48 17.11 180.85 17.25C179.98 17.44 179.98 18.05 180.85 19.75C181.72 21.44 182.62 21.88 184.5 21.5C186.49 21.1 187.28 21.57 188.36 23.75C189.11 25.26 190.46 26.84 191.36 27.25C192.26 27.66 192.84 28.56 192.64 29.25C192.44 29.94 192 31.4 191.66 32.5C191.14 34.17 191.5 34.5 193.77 34.48C195.27 34.47 197.06 34.13 197.75 33.73C198.51 33.29 199 30.84 199 22L204.25 19.62C207.14 18.32 209.8 16.63 210.18 15.87C210.61 15 209.97 14.04 208.43 13.25C206.54 12.28 206.17 11.53 206.75 9.9C207.35 8.22 208.05 7.92 210.25 8.4C212.41 8.87 212.89 8.68 212.5 7.5C212.22 6.67 212.68 5.78 213.5 5.5C214.32 5.22 215.07 4.27 215.15 3.38C215.26 2.19 215.86 1.96 217.4 2.49ZM154.49 54.25C155.86 54.94 157.09 56.23 157.24 57.12C157.44 58.36 158.75 58.77 162.75 58.87C165.64 58.94 168.01 58.66 168.02 58.25C168.04 57.84 166.91 56.49 165.52 55.25C164.14 54.01 162.32 53.23 161.5 53.5C160.68 53.77 159.44 53.4 158.75 52.67C158.06 51.94 156.49 50.82 155.25 50.17C154.01 49.53 152.78 49 152.5 49C152.22 49 152 49.9 152 51C152 52.13 153.08 53.54 154.49 54.25ZM232.83 53.67C232.94 54.54 233.78 54.8 235.25 54.41C236.63 54.05 237.77 54.34 238.19 55.16C238.58 55.9 239.14 56.16 239.44 55.75C239.75 55.34 239.21 54.1 238.25 52.99C237.29 51.89 236.01 50.99 235.42 50.99C234.82 51 233.96 51.3 233.5 51.67C233.04 52.03 232.74 52.93 232.83 53.67ZM224 72C225.33 72 225.75 71.58 225.27 70.75C224.86 70.06 224.41 69.16 224.27 68.75C224.12 68.34 224 67.55 224 67C224 66.45 223.55 66 223 66C222.45 66 222 67.35 222 69C222 71.33 222.44 72 224 72ZM170.5 72.09C171.05 72.09 172.08 71.39 172.78 70.54C173.48 69.7 173.64 69 173.14 69C172.64 69 171.61 69.7 170.86 70.54C170.11 71.39 169.95 72.09 170.5 72.09Z"},{m:"fs",d:"M176.78 95.02C179.26 95.04 181.26 95.78 182.78 97.27C184.05 98.52 185.04 98.95 185.03 98.25C185.01 97.56 185.68 97 186.5 97C187.32 97 188 97.67 188 98.5C188 99.33 188.89 100 190 100C191.1 100 192.68 98.88 193.5 97.5C194.32 96.13 195.56 94.99 196.25 94.98C196.94 94.97 198.51 95.87 199.75 96.98C201.09 98.18 202.65 98.74 203.61 98.35C204.6 97.94 205.56 98.33 206.11 99.35C206.6 100.26 207.45 100.78 208 100.5C208.55 100.22 209.15 100.9 209.32 102C209.5 103.1 209.95 104 210.32 104C210.7 104 211.89 103.44 212.97 102.75C214.7 101.65 215.37 101.93 218.39 105C221.76 108.41 221.86 108.44 222.42 106.25C222.74 105.01 222.78 103.55 222.5 103C222.22 102.45 222.79 101.56 223.75 101.03C224.71 100.5 225.05 100.05 224.5 100.05C223.95 100.04 224.4 99.39 225.5 98.6C227.03 97.5 227.8 97.44 228.79 98.33C229.51 98.97 230.07 100.06 230.04 100.75C230.02 101.44 230.22 102 230.5 102C230.78 102 233.03 100.65 235.5 99C237.97 97.35 240.68 96.22 241.5 96.5C242.32 96.78 243 97.67 243 98.5C243 99.33 242.55 100.22 242 100.5C241.45 100.78 241 101.45 241 102C241 102.55 242.13 103 243.5 103C244.88 103 246.05 103.56 246.12 104.25C246.18 104.94 246.18 106.06 246.12 106.75C246.05 107.44 246.68 108 247.5 108C248.32 108 249.44 108.79 249.98 109.75C250.84 111.28 251.09 111.31 251.96 110C252.51 109.17 252.74 107.71 252.48 106.75C252.21 105.79 252.45 104.78 253 104.5C253.55 104.22 254 103.1 254 102C254 100.2 254.53 100.04 259.25 100.42C262.14 100.65 265.29 100.88 266.25 100.92C267.21 100.96 268.24 101.79 268.54 102.75C268.84 103.76 268.42 104.82 267.54 105.25C266.69 105.66 266 106.67 266 107.5C266 108.33 266.68 109.22 267.5 109.5C268.32 109.78 268.98 110.56 268.96 111.25C268.93 111.94 269.63 113.17 270.5 114C271.63 115.07 272.08 115.14 272.04 114.25C272.02 113.56 273.01 113.01 274.25 113.03C275.49 113.04 277.18 113.74 278 114.58C279.02 115.62 280.55 115.94 282.75 115.56C284.54 115.25 286 114.55 286 114C286 113.45 287.46 113 289.25 113C291.51 112.99 292.96 113.68 294 115.25C294.82 116.49 295.5 119.3 295.5 121.5C295.5 125.05 295.77 125.5 297.89 125.5C299.65 125.5 300.65 126.37 301.64 128.75C302.39 130.54 303.34 131.99 303.75 131.97C304.16 131.96 305.03 132.97 305.68 134.22C306.6 135.98 306.6 136.78 305.68 137.71C304.77 138.64 304.77 139.27 305.69 140.46C306.35 141.31 307.36 142 307.94 142C308.52 142 309.45 140.88 310 139.5C310.6 138 311.8 137 313 137C314.33 137 315.17 137.83 315.5 139.5C315.77 140.88 316.68 142 317.5 142C318.32 142 319.11 143.01 319.25 144.25C319.43 145.9 320.23 146.57 322.25 146.75C324.04 146.91 325.17 147.7 325.5 149C325.83 150.33 325.33 151.33 324 152C322.16 152.92 322.12 153.18 323.53 155.25C324.36 156.49 324.97 157.95 324.86 158.5C324.76 159.05 325.23 160.85 325.91 162.5C327.07 165.3 326.95 165.66 324.1 168C322.42 169.38 321.04 170.95 321.03 171.5C321.02 172.05 322.35 174.07 324 176C325.64 177.93 327.4 179.5 327.91 179.5C328.42 179.5 329.1 179.84 329.42 180.25C329.74 180.66 329.32 182.35 328.5 184C327.68 185.65 327.34 187.11 327.76 187.25C328.18 187.39 328.41 190.09 328.26 193.25C328.04 198.12 327.54 199.46 325 202C323.35 203.65 322 205.45 322 206C322 206.55 321.55 207.22 321 207.5C320.45 207.78 319.26 207.44 318.36 206.75C317.45 206.06 315.43 205.61 313.86 205.75C312.04 205.91 310.82 205.46 310.5 204.5C310.23 203.68 310.23 202.55 310.5 202C310.77 201.45 311.56 201.02 312.25 201.04C312.94 201.07 314.06 200.51 314.75 199.79C315.5 199.02 315.7 197.76 315.25 196.66C314.75 195.42 313.85 195 312.5 195.36C311.4 195.66 308.25 195.43 305.5 194.84C302.75 194.25 299.94 193.37 299.25 192.88C298.39 192.28 298.3 191.45 298.95 190.25C299.64 188.98 299.43 188.08 298.2 186.98C297.26 186.15 295.71 185.58 294.75 185.73C293.79 185.88 293.01 185.66 293.03 185.25C293.04 184.84 291.92 183.94 290.53 183.25C289.14 182.56 287.89 181.02 287.75 179.83C287.61 178.63 286.82 177.42 286 177.14C284.84 176.74 284.67 177.11 285.25 178.81C285.87 180.62 285.37 181.39 282.4 183.25C279.99 184.76 278.58 185.14 278.15 184.4C277.79 183.79 276.49 182.78 275.25 182.15C273.68 181.35 273.2 180.47 273.66 179.25C274.04 178.25 273.93 177.82 273.41 178.24C272.91 178.65 271.94 178.54 271.25 177.99C270.56 177.45 268.65 177.22 267 177.5C264.34 177.94 264.06 178.31 264.54 180.75C264.88 182.45 264.59 183.78 263.79 184.23C263.08 184.64 260.59 184.98 258.25 184.98C255.47 184.99 253.45 184.4 252.39 183.25C250.88 181.6 250.63 181.6 248.14 183.19C246.69 184.11 244.82 185.82 244 186.98C242.6 188.95 242.65 189.09 244.75 189.05C246.1 189.02 247 189.6 247 190.5C247 191.32 246.32 192.22 245.5 192.5C244.68 192.78 242.76 193.01 241.25 193.01C239.74 193.02 236.98 192.68 235.13 192.26C233.27 191.84 231.36 190.71 230.88 189.75C230.26 188.52 230.61 187.18 232.03 185.25C233.15 183.74 234.05 182.16 234.03 181.75C234.02 181.34 232.43 181 230.5 181C228.06 181 227 181.45 227 182.5C227 183.32 227.34 184 227.75 184.01C228.16 184.01 228.43 185.02 228.33 186.26C228.24 187.49 228.13 188.84 228.08 189.25C228.04 189.66 226.99 190.12 225.75 190.27C224.34 190.45 223.64 191.1 223.87 192.02C224.08 192.84 224.41 194.29 224.62 195.25C224.9 196.52 224.1 197.24 221.75 197.88C219.96 198.36 217.2 198.48 215.61 198.13C213.89 197.76 212.37 196.58 211.86 195.25C211.39 194.01 210.55 192.78 210 192.5C209.45 192.22 209 190.65 209 189C209 187.35 208.55 185.78 208 185.5C207.33 185.17 207.33 184.33 208 183C208.55 181.9 209.79 181.02 210.75 181.04C212.31 181.08 212.34 180.92 211 179.5C210.18 178.63 208.94 177.93 208.25 177.96C207.56 177.98 206.55 178.9 206 180C205.45 181.1 205 182.9 205 184C205 185.1 204.55 186 204 186C203.45 186 203.17 186.79 203.37 187.75C203.58 188.71 203.12 190.4 202.36 191.5C201.59 192.6 200.97 194.29 200.98 195.25C200.99 196.21 201.45 197 202 197C202.55 197 203 197.68 203 198.5C203 199.32 201.99 200.21 200.75 200.47C199.51 200.73 197.6 201.04 196.5 201.16C195.4 201.28 193.6 201.08 192.5 200.71C191.19 200.26 190.41 199 190.25 197.02C190.06 194.74 190.43 193.98 191.75 193.92C193.14 193.86 193.19 193.73 192 193.29C191.18 192.98 189.92 192.23 189.22 191.62C188.19 190.73 188.4 190.05 190.22 188.28C192.05 186.49 192.25 185.81 191.24 184.78C190.55 184.08 189.9 181.93 189.8 180C189.65 177.29 188.97 176.16 186.8 175C185.26 174.18 184.22 172.94 184.5 172.25C184.77 171.56 184.32 171 183.5 171C182.68 171 181.78 171.45 181.5 172C181.22 172.55 181.68 174.35 182.5 176C183.37 177.75 183.72 179.94 183.33 181.25C182.96 182.49 181.94 184.02 181.08 184.66C180.21 185.3 179.16 185.64 178.75 185.41C178.34 185.19 177.34 185.79 176.54 186.75C175.74 187.71 175.31 189.18 175.58 190C175.85 190.82 175.5 192.01 174.79 192.64C174.08 193.26 171.7 194.09 169.5 194.49C166.97 194.94 164.58 194.75 160.5 192.76L163 190.99C164.74 189.76 165.27 188.73 164.75 187.61C164.26 186.55 162.72 186 160.25 186C157.58 185.99 156.14 186.57 155.25 188C154.44 189.3 153.3 189.82 152 189.5C150.9 189.22 149.32 189.45 148.5 190C147.68 190.55 145.88 190.78 144.5 190.5C143.07 190.21 141.93 189.25 141.85 188.25C141.76 187.29 141.65 185.92 141.6 185.21C141.54 184.51 140.82 183.5 140 182.98C139.18 182.46 137.26 182.02 135.75 182.02C134.24 182.01 133 181.55 133 181C133 180.45 132.55 180 132 180C131.45 180 130.78 180.9 130.5 182C130.22 183.1 129.21 184.06 128.25 184.14C127.29 184.22 125.6 184.31 124.5 184.34C123.4 184.37 120.92 183.58 119 182.59C117.08 181.6 114.71 181.07 113.75 181.4C112.79 181.73 112.02 182.79 112.04 183.75C112.07 184.71 111.32 186.18 110.38 187C109.42 187.84 106.67 188.52 104.09 188.56C101.56 188.59 98.83 188.99 98 189.45C96.71 190.17 96.74 190.4 98.25 191.14C99.21 191.61 100 192.9 100 194C100 195.1 99.55 196.22 99 196.5C98.45 196.78 97.55 198.13 97 199.5C96.4 201 95.2 202 94 202C92.9 202 91.21 202.9 90.25 204.01C89.28 205.12 87.27 206.01 85.75 206.01C84.24 206 82.78 205.55 82.5 205C82.2 204.4 81.29 204.61 80.25 205.52C78.83 206.76 78.26 206.81 77.25 205.77C76.56 205.07 76.22 203.71 76.5 202.75C76.81 201.67 76.33 200.76 75.25 200.38C74.29 200.04 71.59 198.91 69.25 197.88C66.16 196.51 64.82 195.25 64.35 193.25C63.99 191.74 64.22 189.26 64.85 187.75C65.73 185.64 65.71 184.48 64.75 182.75C64.06 181.51 63.05 180.19 62.5 179.82C61.85 179.37 61.94 178.24 62.75 176.57C63.65 174.71 64.48 174.19 65.75 174.68C66.71 175.06 68.85 175.34 70.5 175.3C72.15 175.27 73.83 175.75 74.23 176.37C74.72 177.12 74.96 176.99 74.94 176C74.92 175.18 75.61 173.82 76.47 173C77.33 172.18 78.14 170.38 78.26 169.01C78.42 167.4 79.21 166.4 80.5 166.19C81.6 166 82.83 166.45 83.23 167.18C83.78 168.17 83.97 168.19 83.98 167.25C83.99 166.56 82.99 165.33 81.75 164.5C79.63 163.1 79.58 162.78 80.84 159.25C82.04 155.92 81.98 155.13 80.34 152.14C78.51 148.8 78.51 148.78 80.8 147.14C82.06 146.24 83.07 144.71 83.05 143.75C83.02 142.79 83.9 141.55 85 141C86.1 140.45 88.13 140 89.5 140C90.88 140 92.22 139.32 92.5 138.5C92.83 137.5 93.83 137.17 95.5 137.5C97.56 137.91 97.89 137.69 97.35 136.25C96.99 135.29 95.76 133.47 94.6 132.2C92.93 130.37 92.73 129.56 93.63 128.2C94.25 127.27 95.72 126.5 96.88 126.5C98.05 126.5 99.23 125.71 99.5 124.75C99.78 123.79 100.45 123 101 123C101.55 123 102.67 124.01 103.48 125.25C104.35 126.57 104.72 128.53 104.37 130C103.99 131.6 104.27 132.75 105.14 133.2C105.9 133.59 107.29 133.14 108.28 132.2C109.26 131.26 109.82 129.71 109.53 128.75C109.24 127.79 109.9 125.42 111 123.5C112.19 121.43 112.59 119.8 112 119.5C111.37 119.19 111.46 118.4 112.25 117.37C112.94 116.47 114.4 115.35 115.5 114.87C116.6 114.39 118.4 113.44 119.5 112.75C120.6 112.06 121.72 111.15 122 110.72C122.28 110.3 123.29 110.18 124.25 110.47C125.41 110.82 126 112.01 126 114C126 116.5 126.33 116.92 128 116.5C129.13 116.22 131.2 116.89 132.75 118.03C134.26 119.15 136.18 120.02 137 119.97C137.96 119.9 137.67 119.44 136.2 118.68C134.93 118.03 134.03 116.83 134.21 116C134.38 115.17 135.42 114.39 136.51 114.25C137.6 114.11 139.06 112.87 139.75 111.5C140.44 110.12 140.78 108.55 140.5 108C140.22 107.45 140.7 106.44 141.54 105.75C142.39 105.06 143.07 103.94 143.04 103.25C143.02 102.56 143.79 102.01 144.75 102.02C145.71 102.02 147.01 102.36 147.63 102.77C148.26 103.17 148.93 104.35 149.13 105.39C149.39 106.73 150.53 107.4 153 107.65C155.85 107.95 156.69 107.59 157.54 105.76C158.11 104.52 159.01 103.56 159.54 103.63C160.07 103.71 161.63 104.13 163 104.56C165.11 105.23 165.38 105.09 164.75 103.67C164.26 102.57 164.51 101.83 165.5 101.5C166.32 101.22 167 100.33 167 99.5C167 98.5 167.67 98.17 169 98.5C170.33 98.83 171.34 98.33 172.03 97C172.78 95.54 174.05 95.01 176.78 95.02ZM252.83 116.04C252.92 116.57 254.24 117.84 255.75 118.87C257.26 119.9 259.4 121.03 260.5 121.38C262.09 121.9 262.64 121.47 263.19 119.26C263.69 117.26 263.41 116.14 262.19 115.18C261.26 114.45 260.61 113.44 260.75 112.93C260.89 112.42 260.1 111.83 259 111.61C257.9 111.4 256.02 112.09 254.83 113.15C253.64 114.21 252.74 115.51 252.83 116.04ZM148.34 118.56C147.69 119.65 147.53 121.16 147.98 122C148.42 122.83 149.28 123.16 149.89 122.75C150.5 122.34 151 120.88 151 119.5C151 117.5 151.5 117 153.5 117C155.5 117 156 116.5 156 114.5C156 113.13 155.62 112 155.15 112C154.69 112 153.22 113.04 151.9 114.31C150.58 115.58 148.98 117.49 148.34 118.56ZM233 117C233 117.55 233.68 118 234.5 118C235.32 118 236 118.45 236 119C236 119.55 236.5 119.89 237.12 119.75C237.74 119.61 238.64 118.49 239.12 117.25C239.75 115.63 239.65 115.04 238.75 115.14C238.06 115.21 236.49 115.44 235.25 115.64C234.01 115.84 233 116.45 233 117ZM180 122C180.55 122 181 121.33 181 120.5C181 119.67 180.55 119 180 119C179.45 119 179 119.67 179 120.5C179 121.33 179.45 122 180 122ZM279.25 123.45C279.94 123.7 280.85 123.58 281.28 123.2C281.71 122.81 282.04 121.71 282.03 120.75C282.01 119.79 281.64 119 281.2 119C280.76 119 279.86 119.9 279.2 121C278.26 122.57 278.27 123.1 279.25 123.45ZM245.5 125C246.52 125.96 247.16 126.1 247.28 125.37C247.38 124.75 246.79 123.68 245.97 123C245.14 122.32 244.34 122.15 244.19 122.63C244.04 123.11 244.63 124.17 245.5 125ZM202.5 129C203.88 129.55 205.22 130 205.5 130C205.78 130 205.61 129.21 205.13 128.25C204.65 127.29 203.25 126.2 202.01 125.84C200.4 125.38 199.79 125.59 199.88 126.59C199.94 127.37 201.13 128.45 202.5 129ZM139.82 129.47C139.92 129.76 140.56 130.19 141.25 130.42C141.94 130.66 143.25 129.98 144.17 128.92C145.09 127.87 145.54 127.03 145.17 127.06C144.8 127.1 143.41 127.53 142.07 128.03C140.73 128.53 139.72 129.18 139.82 129.47ZM153 139C152.45 139 152.25 139.56 152.56 140.25C152.86 140.94 152.41 142.06 151.56 142.75C150.45 143.63 149.42 143.71 148 143C146.67 142.33 145.83 142.33 145.5 143C145.22 143.55 146.01 144.86 147.25 145.91C148.49 146.96 149.72 147.45 150 146.99C150.28 146.54 151.63 145.89 153 145.54C155.19 144.99 155.5 144.37 155.5 140.54C155.5 138.13 155.15 135.79 154.72 135.33C154.29 134.88 153.96 135.51 153.97 136.75C153.99 137.99 153.55 139 153 139ZM167.25 146.75C165.83 146.91 164.83 147.83 164.55 149.25C164.3 150.49 164.53 151.95 165.05 152.5C165.58 153.05 165.78 154.29 165.5 155.25C165.14 156.53 164.39 156.83 162.75 156.35C160.78 155.78 160.66 155.91 161.75 157.35C162.44 158.26 162.7 159.79 162.34 160.75C161.98 161.71 161.75 162.84 161.84 163.25C161.93 163.66 160.88 164.9 159.5 166C158.13 167.1 157 168.9 157 170C157 171.1 157.45 172.22 158 172.5C158.55 172.78 159.39 174.01 159.86 175.25C160.35 176.54 161.83 177.71 163.34 178C165.21 178.36 165.98 178.07 166.02 177C166.06 176.18 165.51 174.99 164.79 174.38C164.08 173.76 163.16 172.3 162.75 171.13C162.14 169.4 162.77 168.39 166.11 165.75C169.43 163.12 170.12 162.02 169.72 160C169.44 158.6 169.93 156.4 170.84 155C171.73 153.63 172.72 152.28 173.05 152C173.38 151.72 173.95 150.71 174.32 149.75C174.77 148.58 174.17 147.17 172.5 145.5C170.24 143.24 170.15 142.83 171.5 141.25C172.33 140.29 172.67 139.15 172.25 138.72C171.84 138.29 171.02 137.96 170.44 137.97C169.86 137.99 169.41 139.91 169.44 142.25C169.49 145.94 169.2 146.53 167.25 146.75ZM278.5 143C279.32 143 279.77 142.55 279.5 142C279.23 141.45 278.55 141 278 141C277.45 141 277 141.45 277 142C277 142.55 277.68 143 278.5 143ZM203.5 146.5C204.32 146.78 205.45 146.78 206 146.5C206.55 146.22 207 145.55 207 145C207 144.45 206.12 144 205.05 144C203.98 144 202.85 144.45 202.55 145C202.25 145.55 202.68 146.22 203.5 146.5ZM301.86 148.5C302.45 149.05 303.62 149.16 304.46 148.75C305.71 148.14 305.77 147.71 304.75 146.46C304.06 145.61 303.18 144.93 302.8 144.96C302.41 144.98 301.8 145.56 301.44 146.25C301.08 146.94 301.27 147.95 301.86 148.5ZM235 152C236.65 152 238 151.55 238 151C238 150.45 236.99 149.32 235.75 148.49C234.51 147.66 233.16 146.99 232.75 146.99C232.34 147 232 148.13 232 149.5C232 151.61 232.47 152 235 152ZM93.6 152.79C95.15 153.84 95.56 153.84 95.84 152.79C96.03 152.08 95.91 150.94 95.59 150.25C95.27 149.56 94.57 149 94.05 149C93.53 149 92.78 149.56 92.4 150.25C92 150.97 92.51 152.05 93.6 152.79ZM108.76 152.07C108.89 152.58 109.67 153.22 110.5 153.5C111.33 153.78 112.45 154 113 154C113.55 154 113.89 153.37 113.75 152.6C113.61 151.82 113.23 151.02 112.89 150.81C112.56 150.6 111.44 150.59 110.4 150.78C109.36 150.98 108.63 151.56 108.76 152.07ZM185.93 156.07C185.97 156.58 187.24 156.71 188.75 156.36C190.26 156.01 191.72 155.55 192 155.34C192.28 155.12 191.81 154.26 190.96 153.4C189.58 152.01 189.25 152.02 187.64 153.5C186.66 154.4 185.89 155.56 185.93 156.07ZM220.01 161.5C222.38 164.7 222.73 165.76 221.74 166.79C220.75 167.83 220.8 168.08 222 168.06C222.9 168.05 223.8 166.62 224.25 164.52C224.66 162.58 224.75 160.44 224.45 159.75C224.15 159.06 222.26 157.77 220.25 156.88C217.94 155.85 216.68 155.67 216.83 156.38C216.95 156.99 218.39 159.3 220.01 161.5ZM203.36 162C204.65 163.31 204.66 163.95 203.42 167C202.14 170.17 202.17 170.61 203.75 171.64C204.71 172.27 206.11 172.5 206.85 172.14C207.68 171.75 208.02 170.53 207.72 169C207.44 167.63 207.67 165.6 208.21 164.5C208.93 163.05 208.76 161.6 207.6 159.25C206.72 157.46 205.35 156 204.55 156C203.76 156 202.83 157.01 202.49 158.25C202.11 159.63 202.45 161.08 203.36 162ZM238.5 162C239.32 162 240.18 161.66 240.4 161.25C240.63 160.84 239.62 160.44 238.17 160.36C236.71 160.28 235.86 160.61 236.26 161.11C236.67 161.6 237.68 162 238.5 162ZM317.8 187.7C317.92 189.13 318.66 190.01 319.75 190.03C320.71 190.04 321.99 189.03 322.6 187.78C323.2 186.53 323.54 184.94 323.35 184.25C323.16 183.56 322.42 183 321.7 183C320.98 183 319.77 183.54 319 184.2C318.23 184.86 317.69 186.44 317.8 187.7Z"}]
  };


  // Internal strand lines are the single defining feature of the reference hair (penny/kevin/jade):
  // a mostly-smooth silhouette carried by MANY fine flowing lines in tones of the hair colour. The
  // faces.js silhouettes shipped without them, so we generate them here in faces.js 400x600 space and
  // clip to the silhouette union, then ride the same FACES_HAIR_TRANSFORM as the shape.




  // longWaves now renders from a purpose-built procedural silhouette in face-generator.js (Luna's
  // flowing waves); the borrowed faces.js "female1" data above rendered as a blunt-banged bob, so it
  // is excluded here to fall back to the procedural path. (Data kept for reference/other fits.)
  const EXCLUDE = new Set(["longWaves"]);

  /* ===================== Unified hair compositor: piece registry ===================== *
   * One registry for every hair piece — migrated base-style geometry AND the lock catalogue.
   * A definition declares its authoring coordinate space instead of rewriting path data:
   *   portrait256 — final compositor space (drawn locks, longWaves)
   *   faces400    — faces.js silhouettes, mapped by FACES_HAIR_TRANSFORM
   *   lock512     — hand-authored locks, mapped per-instance by lockTransform()
   * parts.mass = fill-bearing silhouette paths ONLY (they define the exterior rim union).
   * Stroke-only paths and the per-style automatic overlays (the old overlayLock dark/shine/lines)
   * are detail/lines: clipped to the owning silhouette, never expanding the rim.               */
  const LONG_WAVE_FRONT_D = "M64.0 96.0C57.7 110.0 50.7 113.7 50.0 124.0C49.3 134.3 60.7 146.0 60.0 158.0C59.3 170.0 46.7 184.0 46.0 196.0C45.3 208.0 55.3 220.0 56.0 230.0C56.7 240.0 46.7 254.7 50.0 256.0C53.3 257.3 70.7 248.0 76.0 238.0C81.3 228.0 81.3 209.7 82.0 196.0C82.7 182.3 79.0 169.3 80.0 156.0C81.0 142.7 84.7 126.7 88.0 116.0C91.3 105.3 95.7 96.8 100.0 92.0C104.3 87.2 109.3 88.2 114.0 87.0C118.7 85.8 123.3 85.0 128.0 85.0C132.7 85.0 137.7 85.8 142.0 87.0C146.3 88.2 149.7 87.2 154.0 92.0C158.3 96.8 164.3 105.3 168.0 116.0C171.7 126.7 175.0 142.7 176.0 156.0C177.0 169.3 173.3 182.3 174.0 196.0C174.7 209.7 174.7 228.0 180.0 238.0C185.3 248.0 202.7 257.3 206.0 256.0C209.3 254.7 199.3 240.0 200.0 230.0C200.7 220.0 210.7 208.0 210.0 196.0C209.3 184.0 196.7 170.0 196.0 158.0C195.3 146.0 206.7 134.3 206.0 124.0C205.3 113.7 198.3 110.0 192.0 96.0C185.7 82.0 178.7 50.7 168.0 40.0C157.3 29.3 141.3 32.0 128.0 32.0C114.7 32.0 98.7 29.3 88.0 40.0C77.3 50.7 70.3 82.0 64.0 96.0Z";

  const HAIR_PIECES = {};

  // Migrated base styles: original faces.js path data, coordinate space declared (not rewritten).
  const BASE_LABELS = {
    messy: "Messy Base", cropped: "Cropped Base", sidePart: "Side Part Base", bob: "Bob Base",
    curls: "Curls Base", coily: "Coily Base", locs: "Locs Base", longWaves: "Long Waves Base"
  };
  Object.keys(HAIR).filter((k) => !EXCLUDE.has(k)).forEach((key) => {
    const paths = HAIR[key];
    const lock = LOCKS[LOCK_FOR[key]];
    HAIR_PIECES[`base:${key}`] = {
      key: `base:${key}`,
      label: BASE_LABELS[key] || `${key} Base`,
      coordinateSpace: "faces400",
      baseTransform: FACES_HAIR_TRANSFORM,
      strokeWidth: STROKE,
      parts: {
        mass: paths.filter((p) => p.m === "fs" || p.m === "f").map((p) => p.d),
        fill: paths.filter((p) => p.m === "fs" || p.m === "f").map((p) => p.d),
        detail: [],
        lines: paths.filter((p) => p.m !== "fs" && p.m !== "f").map((p) => p.d)
      },
      // the old overlayLock content, now owned detail: dark/shine/lines of the mapped lock,
      // in lock512 space with the style's overlay transform, clipped to this piece's mass.
      overlay: lock ? {
        space: "lock512",
        transform: LOCK_TF[key] || DEFAULT_LOCK_TF,
        dark: lock.dark || [],
        shine: lock.shine || [],
        lines: lock.lines || []
      } : null,
      defaultZone: "front"
    };
  });
  HAIR_PIECES["base:longWaves"] = {
    key: "base:longWaves",
    label: BASE_LABELS.longWaves,
    coordinateSpace: "portrait256",
    baseTransform: null,
    strokeWidth: 4.1,
    parts: { mass: [LONG_WAVE_FRONT_D], fill: [LONG_WAVE_FRONT_D], detail: [], lines: [] },
    overlay: null,
    defaultZone: "front"
  };
  // The lock catalogue as pieces (geometry shared with LOCKS; instance transform via lockTransform).
  LOCK_CATALOG.forEach(([key, label]) => {
    const lock = LOCKS[key];
    HAIR_PIECES[`lock:${key}`] = {
      key: `lock:${key}`,
      label,
      coordinateSpace: "lock512",
      baseTransform: null,
      strokeWidth: 5.8,
      parts: {
        mass: lock.hair || [],
        fill: lock.hair || [],
        detail: [
          ...(lock.dark || []).map((d) => ({ d, role: "dark" })),
          ...(lock.shine || []).map((d) => ({ d, role: "shine" }))
        ],
        lines: lock.lines || []
      },
      overlay: null,
      defaultZone: "front"
    };
  });

  /* Presets: each advertised base style resolves to an ordered list of base layers.
   * bald is the empty preset; hijab is headwear (an accessory), not hair — also empty. */
  const HAIR_PRESETS = {
    messy: { layers: [{ piece: "base:messy", zone: "front" }] },
    cropped: { layers: [{ piece: "base:cropped", zone: "front" }] },
    sidePart: { layers: [{ piece: "base:sidePart", zone: "front" }] },
    bob: { layers: [{ piece: "base:bob", zone: "front" }] },
    curls: { layers: [{ piece: "base:curls", zone: "front" }] },
    coily: { layers: [{ piece: "base:coily", zone: "front" }] },
    locs: { layers: [{ piece: "base:locs", zone: "front" }] },
    longWaves: { layers: [{ piece: "base:longWaves", zone: "front" }] },
    bald: { layers: [] },
    hijab: { layers: [] }
  };


  /* ---- Designed presets (2026-07-19): multi-piece styles Joel promoted from the Hair Studio's
   * DESIGNED catalogue (composed from his rated yays). Every layer is a normal base layer —
   * editable, reorderable, resettable in the Hair Designer. Drawn components (top-bun spiral,
   * nape tail, question curl, swoop fringe) are his own yay pieces, in raw portrait256 space. ---- */
  const DESIGN_TOP_BUN = { d: "M127.5 55.8C126.5 56.2 123.3 57.8 121.6 58.2C119.8 58.6 118.5 58.5 117.0 58.3C115.5 58.1 113.9 57.6 112.6 57.0C111.3 56.4 110.0 55.5 109.0 54.6C108.0 53.7 107.1 52.7 106.5 51.6C105.8 50.6 105.4 49.5 105.2 48.5C105.0 47.6 105.0 46.6 105.2 45.9C105.3 45.2 105.6 44.6 106.0 44.2C106.3 43.8 106.7 43.6 107.1 43.4C107.5 43.3 107.9 43.3 108.4 43.4C108.8 43.5 109.3 43.6 109.8 43.9C110.3 44.2 110.9 44.6 111.4 45.2C111.8 45.7 112.3 46.4 112.7 47.2C113.0 47.9 113.3 48.8 113.4 49.7C113.6 50.6 113.5 51.6 113.4 52.5C113.3 53.4 113.0 54.3 112.6 55.1C112.2 55.9 111.6 56.6 111.1 57.2C110.5 57.9 109.8 58.4 109.1 58.9C108.4 59.3 107.6 59.6 106.8 59.8C106.1 60.0 105.3 60.1 104.6 60.1C103.9 60.1 103.2 60.0 102.6 59.9C102.0 59.8 101.4 59.1 101.0 59.4C100.5 59.7 99.8 61.3 99.9 61.9C100.1 62.5 101.3 62.6 102.1 62.9C103.0 63.2 103.9 63.4 104.9 63.5C105.9 63.6 107.1 63.5 108.2 63.3C109.3 63.1 110.5 62.7 111.5 62.1C112.6 61.5 113.7 60.8 114.7 59.9C115.6 58.9 116.5 57.8 117.1 56.6C117.7 55.4 118.3 54.0 118.5 52.6C118.8 51.2 118.9 49.7 118.7 48.2C118.6 46.8 118.2 45.3 117.6 43.9C116.9 42.6 116.1 41.2 115.0 40.2C114.0 39.1 112.7 38.1 111.3 37.5C109.9 37.0 108.3 36.6 106.7 36.7C105.2 36.8 103.6 37.3 102.2 38.1C100.9 38.9 99.7 40.1 98.8 41.5C98.0 42.9 97.4 44.6 97.2 46.3C97.0 48.0 97.1 49.9 97.5 51.7C98.0 53.5 98.8 55.5 99.9 57.2C101.0 59.0 102.4 60.8 104.1 62.2C105.8 63.7 107.9 65.1 110.1 66.0C112.3 67.0 114.8 67.7 117.3 68.0C119.8 68.3 122.8 68.1 125.1 67.7C127.4 67.3 130.1 66.0 131.1 65.6Z", strokes: ["M125.2 54.8C123.3 55.5 117.4 58.4 114.1 58.9C110.9 59.3 107.7 58.5 105.9 57.4C104.0 56.3 103.0 54.1 102.9 52.3C102.8 50.5 104.0 48.2 105.2 46.8C106.4 45.4 108.6 44.1 110.2 43.8C111.7 43.4 113.6 43.8 114.6 44.6C115.5 45.5 116.1 47.2 115.9 48.7C115.7 50.3 114.7 52.4 113.5 54.0C112.3 55.6 110.4 57.2 108.7 58.2C107.1 59.3 105.1 59.9 103.7 60.2C102.2 60.6 100.7 60.2 100.1 60.2"] };
  const DESIGN_QUESTION_CURL = { d: "M131.7 113.2C133.2 113.7 137.9 116.0 140.6 116.6C143.2 117.2 145.3 117.0 147.5 116.7C149.8 116.4 152.0 115.6 154.0 114.7C156.0 113.8 157.9 112.5 159.4 111.1C160.9 109.8 162.2 108.1 163.1 106.6C164.0 105.1 164.6 103.4 164.9 102.0C165.2 100.5 165.1 99.1 164.9 98.1C164.7 97.0 164.1 96.2 163.6 95.6C163.1 95.1 162.5 94.9 161.9 94.7C161.4 94.6 160.8 94.7 160.2 94.9C159.7 95.0 159.0 95.3 158.4 95.7C157.7 96.2 156.9 96.8 156.3 97.6C155.6 98.3 155.0 99.3 154.5 100.4C154.1 101.5 153.7 102.8 153.6 104.0C153.4 105.3 153.5 106.7 153.7 107.9C153.9 109.2 154.4 110.4 155.0 111.5C155.5 112.6 156.3 113.7 157.1 114.5C158.0 115.4 159.0 116.1 160.0 116.7C160.9 117.2 162.0 117.6 163.1 117.9C164.1 118.1 165.2 118.2 166.1 118.2C167.1 118.2 168.0 118.0 168.8 117.9C169.6 117.7 170.2 116.2 170.9 117.1C171.7 118.0 173.7 121.9 173.5 123.2C173.3 124.5 171.3 124.4 169.9 124.8C168.6 125.2 167.0 125.6 165.4 125.6C163.8 125.7 162.0 125.6 160.3 125.2C158.5 124.8 156.7 124.2 155.0 123.2C153.3 122.3 151.6 121.0 150.2 119.6C148.8 118.1 147.4 116.3 146.4 114.5C145.5 112.6 144.7 110.4 144.3 108.2C143.9 106.0 143.8 103.6 144.1 101.4C144.4 99.2 145.1 96.8 146.0 94.8C147.0 92.7 148.4 90.6 150.0 89.0C151.7 87.4 153.7 85.9 155.9 85.0C158.0 84.2 160.6 83.7 162.9 83.8C165.3 84.0 167.9 84.8 169.9 86.1C171.9 87.3 173.7 89.3 175.0 91.4C176.2 93.5 177.1 96.1 177.4 98.7C177.7 101.3 177.5 104.1 176.8 106.9C176.1 109.7 174.9 112.6 173.2 115.2C171.6 117.8 169.4 120.5 166.8 122.6C164.2 124.8 161.1 126.8 157.8 128.3C154.5 129.7 150.8 130.8 147.0 131.1C143.3 131.5 138.9 131.2 135.4 130.6C132.0 130.0 127.9 128.0 126.4 127.5Z", strokes: ["M135.2 111.5C138.0 112.5 146.9 116.9 151.7 117.5C156.6 118.2 161.4 117.0 164.2 115.4C167.0 113.7 168.4 110.4 168.5 107.7C168.7 105.1 167.0 101.6 165.2 99.4C163.3 97.3 160.0 95.5 157.7 94.9C155.3 94.4 152.5 95.0 151.1 96.2C149.7 97.5 148.9 100.0 149.1 102.3C149.4 104.7 150.9 107.8 152.7 110.2C154.5 112.6 157.4 115.0 159.8 116.5C162.3 118.1 165.3 119.1 167.4 119.5C169.6 120.0 171.8 119.5 172.7 119.5"] };
  const DESIGN_NAPE_TAIL = { d: "M157.6 135.0C160.3 136.0 169.5 138.6 173.5 140.6C177.6 142.5 179.0 143.6 181.8 146.7C184.6 149.9 188.4 156.0 190.1 159.5C191.8 163.1 192.0 164.4 192.2 168.1C192.4 171.8 192.3 177.0 191.4 181.7C190.4 186.4 187.6 192.7 186.2 196.3C184.9 200.0 184.3 200.5 183.3 203.6C182.4 206.6 181.2 211.5 180.6 214.5C179.9 217.6 180.2 220.9 179.6 222.0C178.9 223.0 177.5 222.7 176.5 221.1C175.5 219.5 174.3 215.0 173.6 212.4C173.0 209.7 173.2 205.5 172.6 205.3C171.9 205.1 170.3 208.8 169.6 211.3C168.9 213.9 168.5 217.0 168.4 220.7C168.2 224.4 169.3 231.8 168.7 233.6C168.2 235.3 166.1 232.8 165.0 231.0C163.8 229.1 162.7 226.6 161.6 222.5C160.5 218.3 159.2 210.6 158.6 206.0C158.0 201.4 158.0 198.8 158.0 194.8C158.1 190.7 158.2 185.8 158.9 181.8C159.6 177.9 161.2 173.7 162.3 170.9C163.3 168.1 164.7 167.5 165.3 165.0C165.9 162.4 166.2 158.5 165.8 155.5C165.5 152.5 164.5 150.2 163.1 146.8C161.7 143.4 158.5 137.0 157.6 135.0Z", strokes: ["M163.9 158.8C166.1 159.8 174.0 162.2 177.2 165.2C180.4 168.2 182.5 172.4 182.9 176.9C183.4 181.5 180.7 188.2 179.7 192.5C178.6 196.7 177.4 200.2 176.5 202.5C175.6 204.8 175.0 206.9 174.3 206.1C173.6 205.2 172.5 198.8 172.1 197.4"] };
  const DESIGN_SWOOP_FRINGE = { d: "M180.9 72.5C178.2 70.0 169.7 60.8 164.7 57.9C159.7 54.9 156.8 54.3 150.8 54.6C144.8 54.9 134.0 57.4 128.6 59.5C123.3 61.5 121.5 64.3 118.6 67.0C115.8 69.8 113.3 72.7 111.4 75.9C109.4 79.1 107.9 83.1 106.9 86.1C106.0 89.1 104.8 90.4 105.9 94.0C106.9 97.6 111.2 104.3 113.3 107.7C115.4 111.2 115.8 112.2 118.6 114.8C121.3 117.3 126.7 121.6 129.8 122.9C133.0 124.3 136.4 124.2 137.2 122.8C138.0 121.4 135.2 117.8 134.8 114.6C134.3 111.3 134.2 106.2 134.6 103.2C135.0 100.3 135.0 99.6 137.1 96.8C139.2 93.9 144.3 88.2 146.9 86.0C149.5 83.8 150.9 84.2 152.5 83.6C154.2 83.0 151.2 80.9 156.7 82.3C162.3 83.8 182.7 92.9 185.8 92.2C188.9 91.6 176.0 80.6 175.3 78.4C174.7 76.1 179.7 78.3 181.9 78.5C184.1 78.7 187.7 80.2 188.5 79.6C189.2 78.9 188.8 77.6 186.5 74.8C184.2 72.0 177.8 65.8 174.4 62.8C171.0 59.8 168.8 58.4 166.1 56.8C163.4 55.2 159.5 53.7 158.2 53.1Z", strokes: ["M166.1 77.6C163.3 75.9 155.2 68.2 149.5 67.7C143.8 67.2 135.8 71.7 131.8 74.6C127.8 77.5 125.9 81.3 125.4 85.1C124.9 88.8 126.8 93.6 128.9 97.0C131.0 100.3 136.0 104.7 138.0 105.3C140.0 106.0 140.2 101.5 140.7 100.7"] };
  const DESIGN_PRESETS = {
    samurai: { layers: [
      { piece: "base:cropped", zone: "front" },
      { ...DESIGN_TOP_BUN, zone: "front" }
    ] },
    question: { layers: [
      { ...DESIGN_QUESTION_CURL, zone: "front", dx: 14, dy: -46 }
    ] },
    napeTail: { layers: [
      { ...DESIGN_NAPE_TAIL, zone: "behind" },
      { piece: "base:messy", zone: "front" }
    ] },
    curtainDome: { layers: [
      { piece: "base:bob", zone: "front" },
      { piece: "lock:softWaveCap", zone: "front", x: 50, y: 28, scale: 0.5, rot: 0 },
      { ...DESIGN_SWOOP_FRINGE, zone: "front" }
    ] },
    crestCap: { layers: [
      { piece: "base:cropped", zone: "front" },
      { piece: "lock:spikyFringe", zone: "front", x: 50, y: 26, scale: 0.52, rot: 0 }
    ] },
    softServe: { layers: [
      { piece: "base:coily", zone: "front" },
      { piece: "lock:roundedPuffSide", zone: "front", x: 50, y: 26, scale: 0.55, rot: 0 },
      { piece: "lock:roundedPuffSide", zone: "front", x: 50, y: 15, scale: 0.4, rot: 0 }
    ] },
    duchess: { layers: [
      { piece: "base:bob", zone: "front" },
      { piece: "lock:longSideLock", zone: "front", x: 55, y: 28, scale: 0.46, rot: 118, mirror: true },
      { piece: "lock:shortCrop", zone: "front", x: 50, y: 22, scale: 0.45, rot: 0 }
    ] },
    princessMane: { layers: [
      { piece: "base:messy", zone: "front" },
      { piece: "lock:extraLongPair", zone: "front", x: 50, y: 34, scale: 0.62, rot: 0 },
      { piece: "lock:curtainBangs", zone: "front", x: 52, y: 28, scale: 0.48, rot: 0 }
    ] },
    topKnotWhip: { layers: [
      { ...DESIGN_NAPE_TAIL, zone: "behind" },
      { piece: "base:cropped", zone: "front" },
      { ...DESIGN_TOP_BUN, zone: "front" }
    ] },
    sideSpill: { layers: [
      { piece: "base:messy", zone: "front" },
      { piece: "lock:longSideLock", zone: "front", x: 56, y: 26, scale: 0.4, rot: -111 },
      { piece: "lock:shortCrop", zone: "front", x: 50, y: 22, scale: 0.4, rot: 0 }
    ] }
  };
  Object.assign(HAIR_PRESETS, DESIGN_PRESETS);

  const warnedPieces = new Set();
  function warnUnknownPiece(piece) {
    if (warnedPieces.has(piece)) return;
    warnedPieces.add(piece);
    if (typeof console !== "undefined" && console.warn) console.warn(`facesHair: unknown hair piece "${piece}" skipped`);
  }

  const num = (v, fallback) => (Number.isFinite(Number(v)) ? Number(v) : fallback);

  // Normalize any base/placed/drawn entry (legacy lock inst, pen-tool inst, preset stub, or an
  // already-normalized layer) into the canonical runtime layer instance. Position semantics are
  // per piece family, preserving all stored data unchanged:
  //   lock:*  — x/y are the lock centre as % of the 256 box (legacy default 50/32)
  //   base:* and drawn — x/y are portrait-px offsets from the authored position (default 0)
  function normalizeHairLayer(layer) {
    if (!layer || typeof layer !== "object") return null;
    const isDrawn = !!layer.d;
    const piece = layer.piece || (isDrawn ? "drawn" : (layer.lock ? `lock:${layer.lock}` : null));
    if (!piece) return null;
    const def = piece === "drawn" ? null : HAIR_PIECES[piece];
    if (piece !== "drawn" && !def) { warnUnknownPiece(piece); return null; }
    const isLock = piece.indexOf("lock:") === 0;
    const uniform = num(layer.scale, 1);
    const out = {
      id: typeof layer.id === "string" && layer.id ? layer.id : null,
      piece,
      origin: layer.origin === "base" ? "base" : "placed",
      preset: layer.preset || null,
      zone: layer.zone === "behind" || layer.zone === "front"
        ? layer.zone
        : (layer.behind ? "behind" : (def ? def.defaultZone : "front")),
      x: isLock ? num(layer.x, 50) : num(layer.x, num(layer.dx, 0)),
      y: isLock ? num(layer.y, 32) : num(layer.y, num(layer.dy, 0)),
      scaleX: num(layer.scaleX, uniform),
      scaleY: num(layer.scaleY, uniform),
      rotation: num(layer.rotation, num(layer.rot, 0)),
      mirror: !!layer.mirror,
      visible: layer.visible !== false,
      lines: layer.lines !== false,
      fill: layer.fill || null,
      outline: layer.outline != null ? layer.outline : null,
      dark: layer.dark || null,
      shine: layer.shine || null,
      line: layer.line || null,
      internalLine: layer.internalLine != null ? !!layer.internalLine : !!layer.outlineForce,
      internalLineWidth: layer.internalLineWidth != null ? num(layer.internalLineWidth, null) : null,
      internalLineColor: layer.internalLineColor || null
    };
    if (isDrawn) {
      out.d = layer.d;
      out.strokes = Array.isArray(layer.strokes) ? layer.strokes : null;
      out.shade = !!layer.shade;
    }
    return out;
  }

  // A preset resolved into normalized base layers with stable IDs (preset + position).
  function getHairPreset(styleKey) {
    const preset = HAIR_PRESETS[styleKey];
    if (!preset) return null;
    return {
      key: styleKey,
      layers: preset.layers
        .map((stub, i) => normalizeHairLayer({ ...stub, id: `base:${styleKey}:${i}`, origin: "base", preset: styleKey }))
        .filter(Boolean)
    };
  }

  // Instance adjustments for base/drawn pieces: portrait-px offset, then rotate/scale/mirror about
  // the head centre, then the definition's declared space mapping.
  function pieceGroupTransform(def, layer) {
    const parts = [];
    if (layer.x || layer.y) parts.push(`translate(${layer.x} ${layer.y})`);
    if (layer.rotation || layer.mirror || layer.scaleX !== 1 || layer.scaleY !== 1) {
      const sx = (layer.mirror ? -1 : 1) * layer.scaleX;
      parts.push(`translate(128 128) rotate(${layer.rotation}) scale(${sx.toFixed(4)} ${layer.scaleY.toFixed(4)}) translate(-128 -128)`);
    }
    if (def && def.baseTransform) parts.push(def.baseTransform);
    return parts.join(" ");
  }

  function baseDetailMarkup(def, layer, ctx) {
    if (!def.overlay) return "";
    const hair = (ctx && ctx.hair) || "#3a2418";
    const dark = layer.dark || lockShade(hair, 0.5);
    const sheen = layer.shine || lockShade(hair, 1.3);
    const line = layer.line || (ctx && ctx.seam) || lockShade(hair, 0.62);
    let inner = "";
    def.overlay.dark.forEach((d) => { inner += `<path d='${d}' fill='${dark}' opacity='0.18'/>`; });
    def.overlay.shine.forEach((d) => { inner += `<path d='${d}' fill='${sheen}' opacity='0.16'/>`; });
    if (layer.lines !== false) {
      def.overlay.lines.forEach((d) => { inner += `<path d='${d}' fill='none' stroke='${line}' stroke-width='5' stroke-linecap='round' stroke-linejoin='round' opacity='0.5'/>`; });
    }
    if (!inner) return "";
    // clip the detail to the OWNING piece's silhouette (the overlayLock behaviour): the clip shape
    // carries the same space mapping as the mass.
    const clipId = `hpiece-${String((ctx && ctx.seed) || "0").replace(/[^a-zA-Z0-9_-]/g, "_")}`;
    const clipBody = def.parts.mass.map((d) => `<path d='${d}'/>`).join("");
    const clipShape = def.baseTransform ? `<g transform='${def.baseTransform}'>${clipBody}</g>` : clipBody;
    return `<defs><clipPath id='${clipId}'>${clipShape}</clipPath></defs>` +
      `<g clip-path='url(#${clipId})'><g transform='${def.overlay.transform}'>${inner}</g></g>`;
  }

  // Base-piece renderer. mass/fill/outline draw the silhouette inside the declared space mapping;
  // detail draws the overlay content OUTSIDE the base transform (it has its own space) but inside
  // the instance adjustments; full = fill + outline + detail together (thumbnails / separate mode).
  function renderBasePiecePart(def, layer, ctx, mode) {
    const partMode = mode || "full";
    const hair = (ctx && ctx.hair) || "#3a2418";
    const fill = layer.fill || (ctx && ctx.fill) || hair;
    const massFill = (ctx && ctx.massFill) || "#000";
    const ink = (ctx && ctx.ink) || "#1f2330";
    const outline = layer.outline === "none" ? "none" : (layer.outline || ink);
    const outlined = hasPaint(outline);
    const outlineScale = clamp(Number(ctx && ctx.outlineScale) || 1, 0.55, 1.75);
    const sw = (def.strokeWidth * outlineScale).toFixed(2);
    const adjust = pieceGroupTransform(null, layer); // instance adjustments only
    const spaceTf = def.baseTransform;
    const inSpace = (body) => (spaceTf ? `<g transform='${spaceTf}'>${body}</g>` : body);
    const wrap = (body) => (adjust ? `<g transform='${adjust}'>${body}</g>` : body);
    const silhouette = (paint, strokePaint, width) => def.parts.mass
      .map((d) => `<path d='${d}' fill='${paint}' stroke='${strokePaint}' stroke-width='${width}' stroke-linejoin='round'/>`)
      .join("");
    const strokeLines = (colour, width, opacity) => def.parts.lines
      .map((d) => `<path d='${d}' fill='none' stroke='${colour}' stroke-width='${width}' stroke-linecap='round' stroke-linejoin='round'${opacity ? ` opacity='${opacity}'` : ""}/>`)
      .join("");

    if (partMode === "mass") {
      const body = def.parts.mass.map((d) => `<path d='${d}' fill='${massFill}' stroke='none'/>`).join("");
      return body ? wrap(inSpace(body)) : "";
    }
    if (partMode === "fill") {
      const body = def.parts.fill.map((d) => `<path d='${d}' fill='${fill}' stroke='none'/>`).join("");
      return body ? wrap(inSpace(body)) : "";
    }
    if (partMode === "outline") {
      if (!outlined) return "";
      const body = def.parts.mass.map((d) => `<path d='${d}' fill='none' stroke='${outline}' stroke-width='${sw}' stroke-linejoin='round' stroke-linecap='round'/>`).join("");
      return body ? wrap(inSpace(body)) : "";
    }
    if (partMode === "seam") {
      const lineC = layer.line || (ctx && ctx.seam) || lockShade(hair, 0.62);
      if (!hasPaint(lineC)) return "";
      const seamWidth = ((ctx && ctx.seamWidth) || 2.4) * outlineScale;
      const seamOpacity = (ctx && ctx.seamOpacity) == null ? 0.22 : ctx.seamOpacity;
      const body = def.parts.mass.map((d) => `<path d='${d}' fill='none' stroke='${lineC}' stroke-width='${seamWidth}' stroke-linejoin='round' stroke-linecap='round' opacity='${seamOpacity}'/>`).join("");
      return body ? wrap(inSpace(body)) : "";
    }
    if (partMode === "detail") {
      const overlay = baseDetailMarkup(def, layer, ctx);
      const extraLines = def.parts.lines.length && outlined ? inSpace(strokeLines(outline, sw)) : "";
      const body = overlay + extraLines;
      return body ? wrap(body) : "";
    }
    // full
    let body = inSpace(silhouette(fill, outlined ? outline : "none", sw) + (outlined ? strokeLines(outline, sw) : ""));
    body += baseDetailMarkup(def, layer, ctx);
    return wrap(body);
  }

  // Lock-catalogue pieces reuse the classic lock renderer through a legacy-instance view so the
  // output stays byte-identical for existing callers.
  function lockInstView(layer) {
    return {
      lock: layer.piece.slice(5),
      x: layer.x, y: layer.y,
      scale: layer.scaleX, scaleY: layer.scaleY,
      rot: layer.rotation, mirror: layer.mirror,
      lines: layer.lines, fill: layer.fill, outline: layer.outline == null ? undefined : layer.outline,
      dark: layer.dark, shine: layer.shine, line: layer.line
    };
  }

  function drawnInstView(layer) {
    return {
      d: layer.d, strokes: layer.strokes || undefined, shade: layer.shade,
      dx: layer.x, dy: layer.y,
      scale: layer.scaleX,
      lines: layer.lines, fill: layer.fill, outline: layer.outline == null ? undefined : layer.outline,
      dark: layer.dark, shine: layer.shine, line: layer.line
    };
  }

  // ---- The generic piece renderer: every base, placed, and drawn layer goes through here. ----
  // modes: mass (solid silhouette for union masks) · fill (no outline) · detail (shading/texture)
  //        · seam (internal articulation) · full (standalone: thumbnails, separate mode)
  function renderHairPiecePart(layerOrInst, ctx, mode) {
    const layer = layerOrInst && layerOrInst.piece && layerOrInst.origin
      ? layerOrInst
      : normalizeHairLayer(layerOrInst);
    if (!layer || layer.visible === false) return "";
    if (layer.piece === "drawn") {
      const inst = drawnInstView(layer);
      const partMode = mode || "full";
      const extra = layer.rotation || layer.mirror || layer.scaleX !== 1 || layer.scaleY !== 1
        ? `translate(128 128) rotate(${layer.rotation}) scale(${((layer.mirror ? -1 : 1) * layer.scaleX).toFixed(4)} ${layer.scaleY.toFixed(4)}) translate(-128 -128)`
        : "";
      const body = partMode === "full" ? renderDrawnLock(inst, ctx) : renderDrawnLockPart(inst, ctx, partMode);
      if (!body) return "";
      return extra ? `<g transform='${extra}'>${body}</g>` : body;
    }
    if (layer.piece.indexOf("lock:") === 0) {
      return renderClassicLockPart(lockInstView(layer), ctx, mode || "full");
    }
    const def = HAIR_PIECES[layer.piece];
    if (!def) { warnUnknownPiece(layer.piece); return ""; }
    return renderBasePiecePart(def, layer, ctx, mode);
  }

  window.facesHair = {
    has: (key) => !EXCLUDE.has(key) && !!HAIR[key],
    styles: Object.keys(HAIR).filter((k) => !EXCLUDE.has(k)),
    // Unified compositor API
    getHairPreset,
    normalizeHairLayer,
    renderHairPiecePart,
    hairPieces: HAIR_PIECES,
    hairPresetKeys: Object.keys(HAIR_PRESETS),
    // Lock designer API (compatibility wrappers over renderHairPiecePart)
    renderLock,
    renderLockPart,
    lockCatalog: LOCK_CATALOG.map(([key, label]) => ({ key, label })),
    lockShade
  };
})();
