...
(function installHairStudio() {
  ...
  function weightedKind(rng, exclude = []) {
    ...
    const total = pool.reduce((s, k) => s + mined.usage[k], 0);
    let roll = rng() * total;
    for (const k of pool) { roll -= mined.usage[k]; if (roll <= 0) return k; }
    return pool[0];
  }
  function coKind(rng, chosen) {
    ...
    const entries = Object.entries(scores).filter(([k]) => mined.byKind[k]);
    const total = entries.reduce((s, [, v]) => s + v, 0) || 1;
    let roll = rng() * total;
    for (const [k, v] of entries) { roll -= v; if (roll <= 0) return k; }
    return weightedKind(rng, chosen);
  }
  function sampleInstance(kind, rng) {
    ...
    if (rng() < 0.5) { inst.mirror = true; inst.x = +(100 - inst.x).toFixed(2); inst.rot = -inst.rot; }
    return inst;
  }

  // ---------- combo generator ----------
  function makeCombo(batch, index) {
    const seed = `hcombo-${batch}-${index}`;
    const rng = seeded(seed);
    const bald = rng() < 0.07;
    const baseStyle = bald ? "bald" : pick(BASE_STYLES, rng);
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      pieces.push(sampleInstance(kind, rng));
    }
    return { id: seed, kind: "combo", baseStyle, hairHex, pieces };
  }
  ...
  function makeMutant(rng) {
    const splice = rng() < 0.3 && KINDS.length > 1;
    let polys, parents;
    if (splice) {
      const a = weightedKind(rng), b = weightedKind(rng, [a]);
      parents = [a, b];
      const top = lockPolys(a).map((p) => p.filter(([, y]) => y < 276));
      const bottom = lockPolys(b).map((p) => p.filter(([, y]) => y >= 236));
      polys = [...top, ...bottom].filter((p) => p.length > 4);
    } else {
      const parent = weightedKind(rng);
      parents = [parent];
      polys = lockPolys(parent);
    }
    const opNames = Object.keys(MUTATIONS);
    const ops = [pick(opNames, rng)];
    if (rng() < 0.5) ops.push(pick(opNames.filter((o) => o !== ops[0]), rng));
    ...
    return { d, strokes: strands, label: `${parents.join("+")} · ${ops.join("+")}${splice ? " · splice" : ""}` };
  }

  // ---------- parametric pieces: spine-grown ribbons ----------
  function makeGrown(rng) {
    const family = pick(["arc", "sCurve", "jHook", "spiral", "zig"], rng);
    const len = 45 + rng() * 65;
    const steps = 22;
    const spine = [];
  ...
  function makePiece(batch, index) {
    const seed = `hpiece-${batch}-${index}`;
    const rng = seeded(seed);
    const def = rng() < 0.5 ? makeMutant(rng) : makeGrown(rng);
    return { id: seed, kind: "piece", ...def };
  }
  ...
  function catalogueMarkup() {
    ...
    els.catMeta.textContent = `${combos.length} combos · ${pieces.length} pieces`;
